 ...
 *
 * 싱글 인스턴스 보장:
 *   시작 시 store/agentsalad.pid로 기존 프로세스 감지 → SIGTERM 종료 → 새로 기동.
 *   PID 파일 없으면 포트 점유 체크로 이중 방어.
 *   정상 종료 시 PID 파일 자동 삭제.
 */
import {
  WEB_UI_ENABLED,
  WEB_UI_HOST,
...
import { getEveryoneTargetId, BUILTIN_CHANNEL_TYPES } from './types.js';
import { cleanupStalePlans } from './plan-executor.js';
import { browserBridge } from './skills/builtin/browser-bridge.js';
import { stopAllPlugins } from './dressing/plugin-loader.js';
import {
  readdirSync,
  statSync,
  existsSync,
...
const PID_FILE = join(STORE_DIR, 'agentsalad.pid');
...
/**
 * 기존 AgentSalad 프로세스 종료 후 새로 시작.
 * 1. PID 파일이 있고 프로세스가 살아있으면 → SIGTERM → 최대 5초 대기 → SIGKILL
 * 2. PID 파일이 없거나 프로세스가 죽었으면 → 포트 점유 체크로 이중 방어
 */
async function ensureSingleInstance(): Promise<void> {
  ...
  if (existsSync(PID_FILE)) {
    try {
      ...
      if (oldPid && isProcessAlive(oldPid)) {
        logger.warn(
          { pid: oldPid },
          'Existing AgentSalad process found — terminating before restart',
        );
        ...
        logger.info({ pid: oldPid }, 'Previous instance terminated');
      }
    } catch {
      // PID 파일 읽기 실패 — 무시하고 진행
    }
    try {
      unlinkSync(PID_FILE);
    } catch {}
  ...
  // PID 파일이 없었거나 삭제 후에도 포트가 점유되어 있으면 경고 후 종료
  if (await isPortInUse(WEB_UI_PORT, WEB_UI_HOST)) {
    logger.error(
      { port: WEB_UI_PORT },
      `Port ${WEB_UI_PORT} still in use after cleanup. Another service may be occupying it — aborting.`,
    );
    process.exit(1);
  }
...
function ensureDefaultEveryoneTargets(): void {
  ...
  const platforms: string[] = [
    ...
    ...pluginTypes.filter((t) => !BUILTIN_CHANNEL_TYPES.includes(t as any)),
  ];
...
async function main(): Promise<void> {
  ...
  // 타임존 초기화: DB 설정 > TZ 환경변수 > 시스템 감지
  const dbTimezone = getServerSetting('timezone');
  initTimezone(dbTimezone);
  const tz = getTimezone();
  const tzSource = getTimezoneSource();
  if (tz === 'UTC' && tzSource !== 'db') {
    logger.warn(
      { timezone: tz, source: tzSource },
      'Timezone is UTC — set TZ env or change in Web UI if unintended',
    );
  } else {
    logger.info(
      { timezone: tz, source: tzSource },
  ...
  // Initialize browser extension WebSocket bridge
  browserBridge.init();
  ...
  if (WEB_UI_ENABLED) {
    webUiServer = startWebUiServer(WEB_UI_HOST, WEB_UI_PORT, {
      ...
      ensureEveryoneTargets: () => ensureDefaultEveryoneTargets(),
      createTarget: (input) => {
      ...
      getBrowserExtensionStatus: () => ({
        connected: browserBridge.isConnected(),
      }),
      onServiceMessage: handleMessage,
    });

    // WebSocket upgrade: /ws/browser → Chrome Extension bridge
    webUiServer.on('upgrade', (req, socket, head) => {
