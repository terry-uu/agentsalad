    ...
    create_cron: tool({
      ...
      execute: async ({
        ...
        if (schedule_type === 'once') {
          ...
          // TZ-naive → 설정 타임존 기준 UTC 변환 후 비교 (시스템 TZ 무관)
          const utcIso = computeOnceNextRun(schedule_time);
        ...
        try {
          ...
          if (schedule_type === 'weekly') {
            ...
            // TZ-naive 시작 시간 → 설정 타임존 기준 UTC 변환
            const startUtc = computeOnceNextRun(schedule_time);
            if (startUtc && new Date(startUtc).getTime() > Date.now()) {
              nextRun = startUtc;
            } else {
