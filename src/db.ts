 ...
 * 내장 드레싱: 저장소에 포함된 local-engine/kakaotalk 플러그인을 첫 실행 시 시드.
 * 스마트 스텝: agent_profiles.smart_step/max_plan_steps + hasNewUserMessages() 인터럽트 감지.
 * server_settings: key-value 서버 전역 설정 (timezone 등).
 */
...
function createSchema(database: Database.Database): void {
  ...
  // 레거시 드레싱스토어 스킬 메타데이터. Store 플로우는 제거되었지만
  // 기존 DB를 깨지 않기 위해 컬럼은 유지하고 신규 스킬은 항상 local로 저장한다.
  safeAlter(
    `ALTER TABLE custom_skills ADD COLUMN source TEXT NOT NULL DEFAULT 'local'`,
  );
  safeAlter(`ALTER TABLE custom_skills ADD COLUMN dressing_skill_id TEXT`);
  safeAlter(`ALTER TABLE custom_skills ADD COLUMN dressing_version TEXT`);
  safeAlter(`ALTER TABLE custom_skills ADD COLUMN dressing_installed_at TEXT`);

  // 드레싱 플러그인 (에이전트/채널 드레싱) 설치 레코드
  database.exec(`
    CREATE TABLE IF NOT EXISTS installed_dressings (
      id TEXT PRIMARY KEY,
      dressing_type TEXT NOT NULL,
      dressing_skill_id TEXT UNIQUE NOT NULL,
      name TEXT NOT NULL,
...
function normalizeSkillsJson(value: unknown): AgentProfile['skills'] {
  ...
  return {
    ...
    file_upload: raw.file_upload === true,
...
const BUNDLED_DRESSINGS: BundledDressingSpec[] = [
  {
    id: 'builtin-local-engine',
    type: 'agent',
    folderName: 'local-engine',
    sourceFolder: 'local-engine',
    providerKey: 'local',
    channelType: null,
  },
  {
    id: 'builtin-kakaotalk',
    type: 'channel',
    folderName: 'kakaotalk',
    sourceFolder: 'kakaotalk',
    providerKey: null,
    channelType: 'kakaotalk',
  },
];

function resolveBundledDressingRoot(): string | null {
  const candidates = [
    path.join(process.cwd(), 'dressings'),
    path.resolve(process.cwd(), '..', 'dressings'),
  ];
...
function ensureBundledDressings(): void {
  const sourceRoot = resolveBundledDressingRoot();
  if (!sourceRoot) {
    logger.warn('Bundled dressings directory not found');
    return;
  }

  const now = new Date().toISOString();
  for (const spec of BUNDLED_DRESSINGS) {
    const sourceDir = path.join(sourceRoot, spec.sourceFolder);
    if (!fs.existsSync(sourceDir)) {
      logger.warn({ sourceDir }, 'Bundled dressing source missing');
      continue;
    }

    const typeDir = spec.type === 'agent' ? 'agent' : 'channel';
    ...
    const metaRaw = readOptionalFile(path.join(sourceDir, 'dressing.json'));
    if (!metaRaw) {
      logger.warn({ sourceDir }, 'Bundled dressing metadata missing');
      continue;
    }
    ...
    const configSchema = readOptionalFile(
      path.join(sourceDir, 'config-schema.json'),
    );

    db.prepare(
      `INSERT INTO installed_dressings
       (id, dressing_type, dressing_skill_id, name, version, provider_key, channel_type, folder_name, config_schema, config_json, installed_by, installed_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NULL, 'builtin', ?)
       ON CONFLICT(dressing_skill_id) DO UPDATE SET
         dressing_type = excluded.dressing_type,
         name = excluded.name,
         ...
         installed_by = 'builtin'`,
    ).run(
      spec.id,
      spec.type,
      spec.id,
      meta.display_name || meta.name || spec.folderName,
      meta.version || '0.0.0',
      spec.providerKey,
      spec.channelType,
      spec.folderName,
...
export function getLlmProviderById(id: string): LlmProvider | undefined {
  ...
  // plugin provider fallback: plugin:xxx → installed_dressings 테이블에서 합성
  if (id.startsWith('plugin:')) {
    const providerKey = id.slice('plugin:'.length);
    const dressing = getInstalledDressingByProviderKey(providerKey);
    if (dressing) {
      return {
        id,
        provider_key: providerKey,
        name: `${dressing.name} (plugin)`,
        base_url: '',
        auth_scheme: 'bearer',
        api_key: 'plugin',
        enabled: 1,
        created_at: dressing.installed_at,
        updated_at: dressing.installed_at,
      };
    }
  }
  return undefined;
...
const CS_COLUMNS =
  'id, name, description, prompt, script, input_schema, tool_name, timeout_ms, folder_name, source, dressing_skill_id, dressing_version, dressing_installed_at, created_at, updated_at';
...
export function createCustomSkill(input: {
  ...
  const now = new Date().toISOString();
  db.prepare(
    `INSERT INTO custom_skills (id, name, description, prompt, script, input_schema, tool_name, timeout_ms, folder_name, source, dressing_skill_id, dressing_version, dressing_installed_at, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
  ).run(
    input.id,
    input.name,
...
export function getServerSetting(key: string): string | null {
  const row = db
...
// ── Installed Dressings (에이전트/채널 플러그인) ──

import type { InstalledDressing, DressingType } from './types.js';

export function listInstalledDressings(
  type?: DressingType,
): InstalledDressing[] {
  if (type) {
    return db
      .prepare(
        `SELECT * FROM installed_dressings WHERE dressing_type = ? ORDER BY installed_at DESC`,
      )
      .all(type) as InstalledDressing[];
  }
  return db
    .prepare(`SELECT * FROM installed_dressings ORDER BY installed_at DESC`)
    .all() as InstalledDressing[];
}

export function getInstalledDressingBySkillId(
  dressingSkillId: string,
): InstalledDressing | undefined {
  return db
    .prepare(`SELECT * FROM installed_dressings WHERE dressing_skill_id = ?`)
    .get(dressingSkillId) as InstalledDressing | undefined;
}

export function getInstalledDressingByProviderKey(
  providerKey: string,
): InstalledDressing | undefined {
  return db
    .prepare(
      `SELECT * FROM installed_dressings WHERE dressing_type = 'agent' AND provider_key = ?`,
    )
    .get(providerKey) as InstalledDressing | undefined;
}

export function getInstalledDressingByChannelType(
  channelType: string,
): InstalledDressing | undefined {
  return db
    .prepare(
      `SELECT * FROM installed_dressings WHERE dressing_type = 'channel' AND channel_type = ?`,
    )
    .get(channelType) as InstalledDressing | undefined;
}

export function createInstalledDressing(
  input: Omit<InstalledDressing, 'installed_at'>,
): void {
  db.prepare(
    `INSERT INTO installed_dressings
     (id, dressing_type, dressing_skill_id, name, version, provider_key, channel_type, folder_name, config_schema, config_json, installed_by, installed_at)
...
export function updateInstalledDressingVersion(
  id: string,
  version: string,
  meta?: { name?: string },
): void {
  if (meta?.name) {
    db.prepare(
      `UPDATE installed_dressings SET version = ?, name = ? WHERE id = ?`,
    ).run(version, meta.name, id);
  } else {
    db.prepare(`UPDATE installed_dressings SET version = ? WHERE id = ?`).run(
      version,
      id,
    );
  }
...
export function updateInstalledDressingConfig(
  id: string,
  configJson: string,
): void {
  db.prepare(`UPDATE installed_dressings SET config_json = ? WHERE id = ?`).run(
    configJson,
    id,
  );
}

export function deleteInstalledDressing(id: string): void {
  db.prepare(`DELETE FROM installed_dressings WHERE id = ?`).run(id);
}
