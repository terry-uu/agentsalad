 ...
 * 멀티채널: Telegram, Discord, Slack + 플러그인 채널 (동적 확장). ChannelType은 string.
 * TargetType: 'user'(DM), 'room'(채널/스레드), 'everyone'(기본 자동 생성 템플릿) 구분.
 * MessageContext: 채널 어댑터가 전달하는 메시지 수신 컨텍스트 (DM/멘션/방/첨부파일 정보).
 * MessageAttachment: 메신저 첨부파일 메타데이터 (URL/파일명/크기/MIME).
 *   file_upload 스킬이 켜진 에이전트만 첨부파일을 수신하여 워크스페이스 저장 + LLM 네이티브 전달.
 * CustomSkill: script(실행) + prompt(사용법) 묶음.
 * InstalledDressing: 에이전트/채널 드레싱 플러그인 설치 레코드.
...
// --- Channel Type (supported messenger platforms + plugins) ---

/** 빌트인: 'telegram' | 'discord' | 'slack'. 플러그인 채널은 동적 문자열. */
export type ChannelType = string;

/** 빌트인 채널 타입 목록. UI 검증 및 아이콘 분기에 사용. */
export const BUILTIN_CHANNEL_TYPES = ['telegram', 'discord', 'slack'] as const;
...
// --- Message Attachment (channel → service router) ---
...
export interface MessageAttachment {
  ...
  /** 파일 크기 (bytes) */
  size: number;
  /** MIME type (image/png, application/pdf 등) */
  mediaType: string;
  /** Slack 전용: 다운로드 시 Authorization 헤더에 사용할 봇 토큰 */
  authToken?: string;
}
...
export interface MessageContext {
  ...
  /** @봇 멘션 여부 (서버 채널 내에서) */
  isMention?: boolean;
  /** 메신저 첨부파일 목록 (file_upload 스킬 ON인 에이전트만 처리) */
...
export interface AgentSkillToggles {
  ...
  file_write: boolean;
  file_list: boolean;
  /** 메신저 첨부파일 수신 → 워크스페이스 저장 + LLM 네이티브 전달 */
  file_upload: boolean;
  web_fetch: boolean;
  web_browse: boolean;
  bash: boolean;
  /** 타임아웃 없는 장시간 터미널 — 코딩 에이전트(claude -p, opencode) 등 */
  terminal: boolean;
...
export interface CustomSkill {
  ...
  /** 스킬 폴더명 (이름 기반, 리네임 추적) */
  folder_name: string;
  /** 스킬 출처: 로컬 직접 생성 vs 드레싱스토어 설치 */
  source: 'local' | 'dressing';
  /** 드레싱스토어 skill UUID (source='dressing'일 때) */
  dressing_skill_id: string | null;
  /** 설치된 드레싱 버전 (source='dressing'일 때) */
  dressing_version: string | null;
  /** 드레싱 설치 시각 ISO (source='dressing'일 때) */
  dressing_installed_at: string | null;
  created_at: string;
  updated_at: string;
}
...
// --- Installed Dressing (에이전트/채널 플러그인) ---

/** 드레싱 플러그인 타입: 에이전트(프로바이더) 또는 채널(메신저) */
export type DressingType = 'agent' | 'channel';

export interface InstalledDressing {
  id: string;
  dressing_type: DressingType;
  dressing_skill_id: string;
  name: string;
  version: string;
  /** 에이전트 드레싱: getModelFactory 조회 키 (e.g. 'ollama') */
  provider_key: string | null;
  /** 채널 드레싱: createChannelByType 조회 키 (e.g. 'kakaotalk') */
  channel_type: string | null;
  /** 플러그인 폴더: store/dressings/{type}/{folder}/ */
  folder_name: string;
  /** 유저 설정 스키마 JSON */
  config_schema: string | null;
  /** 유저 설정 값 JSON */
  config_json: string | null;
  /** 설치한 유저 (Google buyer_id) */
