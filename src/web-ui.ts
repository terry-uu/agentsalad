...
en:{
  ...
  sd_web_browse:'Opens a real browser for clicking, typing, scrolling. Use for login or dynamic sites. Install: <code>npx playwright install</code>',
...
ko:{
  ...
  sd_web_browse:'실제 브라우저를 열어 클릭, 입력, 스크롤 등을 수행합니다. 로그인이나 동적 사이트에 사용합니다. 설치: <code>npx playwright install</code>',
...
ja:{
  ...
  sd_web_browse:'実際のブラウザでクリック、入力、スクロールを実行します。ログインや動的サイトに使います。インストール: <code>npx playwright install</code>',
...
zh:{
  ...
  sd_web_browse:'打开真实浏览器进行点击、输入、滚动。用于登录或动态网站。安装: <code>npx playwright install</code>',
