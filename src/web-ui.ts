 ...
 * Agents 탭: 에이전트 상세 설정 (스킬, 프롬프트) + 플러그인 모델 관리 패널
 * Skills 탭: 기본/커스텀 스킬 카탈로그
 *
 * 플러그인 제네릭 인프라:
 * - 에이전트/채널 플러그인 API: /api/plugins/:providerKey/:action (status/load/unload/browse/config)
 * - has_model_lifecycle 플래그 기반 자동 UI 렌더링 (모델 마운트/언마운트/상태)
 * - renderPluginConfigFields: config-schema.json 기반 폼 자동 생성 (string/number/boolean/select)
 * - show_when 조건부 필드 + applyCfgConditions로 동적 토글
 * - dressing.json 메타데이터: file_extensions, guide_url, guide_text, supports_outbound
 * - 코어에 플러그인별 문자열(provider_key, channel_type) 없음 — 새 플러그인 추가 시 코어 수정 0줄
 *
 * 플러그인 모델 상태 동기화:
 * - load()에서 has_model_lifecycle 프로바이더의 /status를 선행 조회 → _pluginModelStatus 초기화
 * - pluginMount/pluginUnload에서 await renderPluginModelPanel() 후 renderAgentList() 호출
 *   (비동기 완료 전 좌측 리스트가 stale 상태로 그려지던 레이스 컨디션 수정)
 * - pluginSaveConfig의 setTimeout 콜백도 동일하게 await 처리
 */
...
import { computeWeeklyNextRun, parseScheduleDays } from './cron-scheduler.js';
...
import { BUILTIN_CHANNEL_TYPES } from './types.js';
...
export interface WebUiContext {
  ...
  updateManagedChannel: (
    id: string,
    updates: { name?: string; autoSession?: number; configJson?: string },
  ) => void;
  ...
  // Browser extension
  getBrowserExtensionStatus: () => { connected: boolean };
  // Webhook bridge (채널 드레싱 플러그인의 웹훅 수신 시 서비스 라우터로 전달)
...
.news-block{margin:0 0 8px;padding:12px 16px;border:1px solid var(--border);border-radius:var(--r);background:var(--s1);display:flex;align-items:center;gap:12px;cursor:pointer;text-decoration:none;transition:.15s}
.news-block:hover{background:var(--s2);border-color:var(--s3)}
.news-label{flex-shrink:0;font-weight:600;font-size:.78rem;color:var(--t2);display:flex;align-items:center;gap:6px}
.news-track{flex:1;overflow:hidden;position:relative;height:1.3em;font-size:.82rem}
.news-track .news-item{position:absolute;inset:0;display:flex;align-items:center;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;color:var(--t2);opacity:0;transform:translateY(100%);animation:newsSlide 6s ease}
@keyframes newsSlide{0%{transform:translateY(100%);opacity:0}10%{transform:translateY(0);opacity:1}85%{transform:translateY(0);opacity:1}100%{transform:translateY(-100%);opacity:0}}
.community-block{display:flex;gap:8px;padding:0 0 24px}
.comm-card{flex:1;display:flex;align-items:center;justify-content:center;gap:8px;font-size:.78rem;color:var(--t2);text-decoration:none;padding:10px 14px;border:1px solid var(--border);border-radius:var(--r);background:var(--s1);transition:.15s}
.comm-card:hover{color:var(--t1);background:var(--s2);border-color:var(--s3)}
...
.main-content{flex:1;min-width:0;padding:32px 28px 44px;overflow-y:auto;max-height:100vh}
...
.cron-card .cc-ttl{display:none;color:var(--green);font-size:.58rem;font-weight:500;margin-left:2px}
.cron-card:hover .cc-ttl{display:inline}
.blk.cr .blk-ttl{display:none;color:var(--green);font-size:.6rem;font-weight:500;margin-left:4px}
.blk.cr:hover .blk-ttl{display:inline}
...
.d-field>label.check-label input[type="checkbox"]{width:auto;padding:0}
...
.d-field details summary::-webkit-details-marker{display:none}
.d-field details[open] summary svg{transform:rotate(0)}
...
@media(max-width:700px){
  ...
  .main-content{max-height:none;padding:16px 12px 44px}
  .news-block{margin:0 0 8px;padding:10px 12px;gap:8px}
  .community-block{padding:0 0 16px}
  .comm-card{font-size:.72rem;padding:8px 8px}
...
<div class="tab-nav">
  ...
  <div class="tab-nav-r">
    <select class="lang-sel" id="tzSelect" onchange="setTz(this.value)" title="Server Timezone">
  ...
  <div class="split-layout">
    <div class="split-list" id="skillsList">
      ...
      <div style="margin-top:16px;padding-top:12px;border-top:1px solid var(--border)">
        <div class="sec-label" style="font-size:.76rem" id="secBrowserExt"></div>
        <div id="browserExtStatus"></div>
      </div>
      <div style="margin-top:16px;padding-top:12px;border-top:1px solid var(--border)">
...
<div class="community-block">
  <a href="javascript:void(0)" onclick="window.open(_bugReportFormUrl,'_blank')" class="comm-card" style="border-color:var(--red)">
    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
    <span id="commBugReport">Report Bug</span>
  </a>
</div>
</main>
</div><!-- /app-layout -->
...
en:{
  ...
  minutes:'min',hours:'hr',days:'d',remaining:' left',
  ...
  maxPlanSteps:'Max steps',workspace:'Workspace',openFolder:'Open Folder',exportChat:'Export Chat',exportAll:'Export All Chats',
  pluginNoModel:'No model mounted',pluginModelStatus:'Status',pluginModelMount:'Mount',pluginModelUnmount:'Unmount',pluginModelPath:'Path to model file',pluginNoFiles:'No matching files found',pluginMountFirst:'Mount a model first in the plugin panel',
  ...
  missingSkills:'Agent is missing required skills: ',cronNoOutbound:'This channel does not support outbound messages. Cron cannot be attached.',webhookNeedUrl:'Set the URL field in channel config to generate webhook URL.',setupGuide:'Setup Guide',setupGuideCollapse:'Hide Guide',configSaveWarnModel:'Saving settings will unmount the current model. Re-mount after saving.',configHelpMore:'Details',pluginSettings:'Settings',pluginSaved:'Saved',
  ...
  browserExtConnected:'Browser extension connected.',
  browserExtNotConnected:'Browser extension not connected.',
  browserExtExplain:'The web_browse skill controls your real Chrome browser via a Chrome Extension. No headless browser — zero bot detection.',
  browserExtStep1:'Open the extension source folder',
  browserExtStep2:'Open chrome://extensions and enable Developer Mode',
  browserExtStep3:'Click "Load unpacked" and select the extension folder',
  browserExtStep4:'Click the extension icon and connect to your server',
  browserExtDownload:'Open Extension Source',
  browserExtSkillNote:'This skill requires the Agent Salad Browser Extension. Load the extension folder to enable browser control.',
  ...
  botTokenHintSlack:'<ol style="margin:4px 0 0 16px;padding:0;font-size:.78rem;line-height:1.5">'
    ...
    +'<li>Copy the <b>Bot User OAuth Token</b> and paste it here.</li>'
    +'<li><b>App Home</b> → <b>Messages Tab</b> section → check <b>"Allow users to send Slash commands and messages from the messages tab"</b>. Without this, users cannot DM the bot.</li>'
    +'</ol>',
  appTokenHint:'<ol style="margin:4px 0 0 16px;padding:0;font-size:.78rem;line-height:1.5">'
    ...
    +'<li>Copy the generated <b>App-Level Token</b> and paste it here.</li>'
    +'</ol>',
  ...
  sl_file_read:'File Read',sl_file_write:'File Write',sl_file_list:'File List',sl_file_upload:'File Upload',
  sl_web_fetch:'Web Page Fetch',sl_web_browse:'Web Browser',sl_bash:'Terminal Commands',sl_terminal:'Terminal (No Timeout)',
  ...
  sd_file_upload:'Agents can receive files via messenger. Supported: PDF, Images (PNG/JPG/GIF/WebP), Excel (.xlsx/.xls), Word (.docx/.doc), HWP (.hwp/.hwpx).',
  sd_web_fetch:'Fetches text from a web page URL. Use for reading news, documents, etc.',
  sd_web_browse:'Controls your real Chrome browser via the Agent Salad Extension. Performs tasks that Web Fetch alone cannot handle. <b style="color:var(--red)">\\u26A0 Opens your actual browser.</b> Only enable for yourself or trusted users.',
  ...
  sd_terminal:'Runs commands with NO timeout. Use for coding agents (Claude Code, OpenCode), builds, or any long-running task. <b style="color:var(--red)">⚠ Full device access, no time limit.</b> Only enable for yourself or trusted users.',
  ...
  // Provider hints
  provHintAnthropic:'e.g. claude-sonnet-4-6',provHintOpenai:'e.g. gpt-5.4',
  provHintGoogle:'e.g. gemini-2.5-flash',provHintGroq:'e.g. llama-3.3-70b-versatile',provHintOpenrouter:'e.g. anthropic/claude-sonnet-4-6',
  provHintOpencode:'e.g. claude-sonnet-4-6',provHintMoonshot:'e.g. kimi-k2.5',provHintGlm:'e.g. glm-5',
  ...
  linkAccount:'Link Account',disconnect:'Disconnect',newsLabel:'News',
  commKakao:'Korean Community (KakaoTalk)',commDiscord:'Discord Community (Global)',commBugReport:'Report Bug',
},
ko:{
  ...
  minutes:'분',hours:'시간',days:'일',remaining:' 남음',
  ...
  maxPlanSteps:'최대 단계 수',workspace:'워크스페이스',openFolder:'폴더 열기',exportChat:'대화 내보내기',exportAll:'전체 대화 내보내기',
  pluginNoModel:'마운트된 모델 없음',pluginModelStatus:'상태',pluginModelMount:'마운트',pluginModelUnmount:'언마운트',pluginModelPath:'모델 파일 경로',pluginNoFiles:'일치하는 파일이 없습니다',pluginMountFirst:'플러그인 패널에서 모델을 먼저 마운트하세요',
  ...
  missingSkills:'에이전트에 필요한 스킬이 없습니다: ',cronNoOutbound:'이 채널은 아웃바운드 메시지를 지원하지 않아 크론을 연결할 수 없습니다.',webhookNeedUrl:'채널 설정에서 URL을 입력하면 웹훅 주소가 생성됩니다.',setupGuide:'셋업 가이드',setupGuideCollapse:'가이드 접기',configSaveWarnModel:'설정 변경 시 마운트된 모델이 해제됩니다. 저장 후 다시 마운트하세요.',configHelpMore:'자세히',pluginSettings:'설정',pluginSaved:'저장됨',
  ...
  browserExtension:'브라우저 익스텐션',
  browserExtConnected:'브라우저 익스텐션이 연결되어 있습니다.',
  browserExtNotConnected:'브라우저 익스텐션이 연결되지 않았습니다.',
  browserExtExplain:'web_browse 스킬은 Chrome 익스텐션을 통해 실제 브라우저를 제어합니다. 헤드리스 브라우저가 아니라 봇 감지에 걸리지 않습니다.',
  browserExtStep1:'익스텐션 소스 폴더 열기',
  browserExtStep2:'chrome://extensions를 열고 개발자 모드 활성화',
  browserExtStep3:'"압축해제된 확장 프로그램을 로드합니다" 클릭 후 extension 폴더 선택',
  browserExtStep4:'익스텐션 아이콘 클릭 후 서버에 연결',
  browserExtDownload:'익스텐션 소스 열기',
  browserExtSkillNote:'이 스킬은 Agent Salad 브라우저 익스텐션이 필요합니다. extension 폴더를 로드하면 브라우저 제어가 활성화됩니다.',
  updateAvailable:'새 버전이 있습니다: v{version}',
  updateDownload:'다운로드',
  ...
  botTokenHintSlack:'<ol style="margin:4px 0 0 16px;padding:0;font-size:.78rem;line-height:1.5">'
    ...
    +'<li>설치 후 보이는 <b>Bot User OAuth Token</b>을 여기 붙여넣으세요.</li>'
    +'<li><b>App Home</b> → <b>Messages Tab</b> 섹션 → <b>"Allow users to send Slash commands and messages from the messages tab"</b>를 체크하세요. 이걸 켜야 사용자가 봇에게 DM을 보낼 수 있습니다.</li>'
    +'</ol>',
  appTokenHint:'<ol style="margin:4px 0 0 16px;padding:0;font-size:.78rem;line-height:1.5">'
    +'<li><b>Basic Information</b> → <b>App-Level Tokens</b> → <b>Generate Token and Scopes</b></li>'
    +'<li>이름은 아무거나 넣고, 스코프는 <code>connections:write</code> 하나만 추가하세요.</li>'
    +'<li>생성된 <b>App-Level Token</b>을 여기 붙여넣으세요.</li>'
    +'</ol>',
  ...
  phApiKey:'API 키',
  sl_file_read:'파일 읽기',sl_file_write:'파일 쓰기',sl_file_list:'파일 목록',sl_file_upload:'파일 업로드',
  sl_web_fetch:'웹 페이지 가져오기',sl_web_browse:'웹 브라우저',sl_bash:'터미널 명령어',sl_terminal:'터미널 (무제한)',
  ...
  sd_file_upload:'에이전트가 메신저를 통해 파일을 받을 수 있습니다. 지원 포맷: PDF, 이미지(PNG/JPG/GIF/WebP), Excel(.xlsx/.xls), Word(.docx/.doc), 한글(.hwp/.hwpx).',
  sd_web_fetch:'URL을 입력하면 해당 웹 페이지의 텍스트를 가져옵니다. 뉴스, 문서 등을 읽을 때 사용합니다.',
  sd_web_browse:'Agent Salad 익스텐션을 통해 실제 Chrome 브라우저를 제어합니다. 웹 페치로 수행할 수 없는 작업을 직접 브라우저를 열어 처리합니다. <b style="color:var(--red)">\\u26A0 직접 브라우저가 열립니다.</b> 본인 또는 신뢰하는 사람에게만 서비스하세요.',
  sd_bash:'서버에서 셸 명령어를 실행합니다. 스크립트 실행, 패키지 설치 등에 사용합니다. <b style="color:var(--red)">⚠ 디바이스 전체 접근 가능.</b> 본인 또는 신뢰하는 사람에게만 서비스하세요.',
  sd_terminal:'타임아웃 없이 명령을 실행합니다. 코딩 에이전트(Claude Code, OpenCode), 빌드 등 장시간 작업에 사용합니다. <b style="color:var(--red)">⚠ 디바이스 전체 접근, 시간 제한 없음.</b> 본인 또는 신뢰하는 사람에게만 서비스하세요.',
  ...
  provHintAnthropic:'예: claude-sonnet-4-6',provHintOpenai:'예: gpt-5.4',
  provHintGoogle:'예: gemini-2.5-flash',provHintGroq:'예: llama-3.3-70b-versatile',provHintOpenrouter:'예: anthropic/claude-sonnet-4-6',
  provHintOpencode:'예: claude-sonnet-4-6',provHintMoonshot:'예: kimi-k2.5',provHintGlm:'예: glm-5',
  ...
  linkAccount:'드레싱 스토어 연결하기',disconnect:'연결 해제',newsLabel:'소식',
  commKakao:'한국 사용자 모임 (카카오톡)',commDiscord:'디스코드 커뮤니티 (글로벌)',commBugReport:'오류 보고',
},
ja:{
  ...
  minutes:'分',hours:'時間',days:'日',remaining:' 残り',
  ...
  maxPlanSteps:'最大ステップ数',workspace:'ワークスペース',openFolder:'フォルダを開く',exportChat:'チャットエクスポート',exportAll:'全チャットエクスポート',
  pluginNoModel:'マウントされたモデルなし',pluginModelStatus:'ステータス',pluginModelMount:'マウント',pluginModelUnmount:'アンマウント',pluginModelPath:'モデルファイルのパス',pluginNoFiles:'一致するファイルが見つかりません',pluginMountFirst:'プラグインパネルで先にモデルをマウントしてください',
  ...
  missingSkills:'エージェントに必要なスキルがありません: ',cronNoOutbound:'このチャンネルはアウトバウンドメッセージに対応していないため、クロンを接続できません。',webhookNeedUrl:'チャンネル設定でURLを入力するとWebhookアドレスが生成されます。',setupGuide:'セットアップガイド',setupGuideCollapse:'ガイドを隠す',configSaveWarnModel:'設定を変更するとマウント中のモデルが解除されます。保存後に再マウントしてください。',configHelpMore:'詳細',pluginSettings:'設定',pluginSaved:'保存済み',
  ...
  browserExtConnected:'ブラウザ拡張機能が接続されています。',
  browserExtNotConnected:'ブラウザ拡張機能が接続されていません。',
  browserExtExplain:'web_browseスキルはChrome拡張機能を通じて実際のブラウザを制御します。ヘッドレスブラウザではないため、ボット検知に引っかかりません。',
  browserExtStep1:'拡張機能のソースフォルダを開く',
  browserExtStep2:'chrome://extensionsを開いてデベロッパーモードを有効化',
  browserExtStep3:'「パッケージ化されていない拡張機能を読み込む」をクリックし、extensionフォルダを選択',
  browserExtStep4:'拡張機能アイコンをクリックしてサーバーに接続',
  browserExtDownload:'拡張機能ソースを開く',
  browserExtSkillNote:'このスキルにはAgent Saladブラウザ拡張機能が必要です。extensionフォルダを読み込むとブラウザ制御が有効になります。',
  ...
  botTokenHintSlack:'<ol style="margin:4px 0 0 16px;padding:0;font-size:.78rem;line-height:1.5">'
    ...
    +'<li>表示される<b>Bot User OAuth Token</b>をここに貼り付けてください。</li>'
    +'<li><b>App Home</b> → <b>Messages Tab</b> → <b>"Allow users to send Slash commands and messages from the messages tab"</b> にチェックを入れてください。これがないとユーザーがボットにDMを送れません。</li>'
    +'</ol>',
  appTokenHint:'<ol style="margin:4px 0 0 16px;padding:0;font-size:.78rem;line-height:1.5">'
    +'<li><b>Basic Information</b> → <b>App-Level Tokens</b> → <b>Generate Token and Scopes</b></li>'
    +'<li>名前は何でもOK、スコープは<code>connections:write</code>を1つ追加。</li>'
    +'<li>生成された<b>App-Level Token</b>をここに貼り付けてください。</li>'
    +'</ol>',
  ...
  phApiKey:'APIキー',
  sl_file_read:'ファイル読み取り',sl_file_write:'ファイル書き込み',sl_file_list:'ファイル一覧',sl_file_upload:'ファイルアップロード',
  sl_web_fetch:'Webページ取得',sl_web_browse:'Webブラウザ',sl_bash:'ターミナルコマンド',sl_terminal:'ターミナル（無制限）',
  ...
  sd_file_upload:'メッセンジャー経由でファイルを受信できます。対応形式: PDF、画像(PNG/JPG/GIF/WebP)、Excel(.xlsx/.xls)、Word(.docx/.doc)、HWP(.hwp/.hwpx)。',
  sd_web_fetch:'URLからWebページのテキストを取得します。ニュースやドキュメントの閲覧に使います。',
  sd_web_browse:'Agent Salad拡張機能を通じて実際のChromeブラウザを制御します。Web Fetchでは処理できないタスクを直接ブラウザで実行します。<b style="color:var(--red)">\\u26A0 実際のブラウザが開きます。</b>自分または信頼できる人にのみ使用してください。',
  sd_bash:'サーバーでシェルコマンドを実行します。スクリプト実行やパッケージインストールなどに使います。<b style="color:var(--red)">⚠ デバイス全体にアクセス可能。</b>自分または信頼できる人にのみ使用してください。',
  sd_terminal:'タイムアウトなしでコマンドを実行します。コーディングエージェント（Claude Code、OpenCode）やビルドなどに使います。<b style="color:var(--red)">⚠ デバイス全体にアクセス可能、時間制限なし。</b>自分または信頼できる人にのみ使用してください。',
  ...
  provHintAnthropic:'例: claude-sonnet-4-6',provHintOpenai:'例: gpt-5.4',
  provHintGoogle:'例: gemini-2.5-flash',provHintGroq:'例: llama-3.3-70b-versatile',provHintOpenrouter:'例: anthropic/claude-sonnet-4-6',
  provHintOpencode:'例: claude-sonnet-4-6',provHintMoonshot:'例: kimi-k2.5',provHintGlm:'例: glm-5',
  ...
  linkAccount:'',disconnect:'',newsLabel:'',
  commKakao:'韓国ユーザーコミュニティ (KakaoTalk)',commDiscord:'Discordコミュニティ (グローバル)',commBugReport:'バグ報告',
},
zh:{
  ...
  minutes:'分钟',hours:'小时',days:'天',remaining:' 剩余',
  ...
  maxPlanSteps:'最大步骤数',workspace:'工作空间',openFolder:'打开文件夹',exportChat:'导出聊天',exportAll:'导出全部聊天',
  pluginNoModel:'未挂载模型',pluginModelStatus:'状态',pluginModelMount:'挂载',pluginModelUnmount:'卸载',pluginModelPath:'模型文件路径',pluginNoFiles:'未找到匹配文件',pluginMountFirst:'请先在插件面板中挂载模型',
  ...
  missingSkills:'代理缺少所需技能: ',cronNoOutbound:'此频道不支持外发消息，无法绑定定时任务。',webhookNeedUrl:'在频道设置中输入URL后将生成Webhook地址。',setupGuide:'设置指南',setupGuideCollapse:'隐藏指南',configSaveWarnModel:'更改设置将卸载当前模型。保存后请重新挂载。',configHelpMore:'详细',pluginSettings:'设置',pluginSaved:'已保存',
  ...
  browserExtConnected:'浏览器扩展已连接。',
  browserExtNotConnected:'浏览器扩展未连接。',
  browserExtExplain:'web_browse技能通过Chrome扩展控制真实浏览器，不使用无头浏览器，不会触发机器人检测。',
  browserExtStep1:'打开扩展源码文件夹',
  browserExtStep2:'打开 chrome://extensions 并启用开发者模式',
  browserExtStep3:'点击"加载已解压的扩展程序"并选择 extension 文件夹',
  browserExtStep4:'点击扩展图标连接到服务器',
  browserExtDownload:'打开扩展源码',
  browserExtSkillNote:'此技能需要Agent Salad浏览器扩展。加载 extension 文件夹后即可控制浏览器。',
  updateAvailable:'新版本可用: v{version}',
  updateDownload:'下载',
  ...
  botTokenHintSlack:'<ol style="margin:4px 0 0 16px;padding:0;font-size:.78rem;line-height:1.5">'
    ...
    +'<li>复制显示的 <b>Bot User OAuth Token</b> 并粘贴到这里。</li>'
    +'<li><b>App Home</b> → <b>Messages Tab</b> → 勾选 <b>"Allow users to send Slash commands and messages from the messages tab"</b>。不勾选的话用户无法向机器人发送私信。</li>'
    +'</ol>',
  appTokenHint:'<ol style="margin:4px 0 0 16px;padding:0;font-size:.78rem;line-height:1.5">'
    +'<li><b>Basic Information</b> → <b>App-Level Tokens</b> → <b>Generate Token and Scopes</b></li>'
    +'<li>名称随意，添加范围 <code>connections:write</code>。</li>'
    +'<li>复制生成的 <b>App-Level Token</b> 并粘贴到这里。</li>'
    +'</ol>',
  ...
  phApiKey:'API密钥',
  sl_file_read:'文件读取',sl_file_write:'文件写入',sl_file_list:'文件列表',sl_file_upload:'文件上传',
  sl_web_fetch:'网页获取',sl_web_browse:'网页浏览器',sl_bash:'终端命令',sl_terminal:'终端（无限制）',
  ...
  sd_file_upload:'通过Messenger接收文件。支持格式: PDF、图片(PNG/JPG/GIF/WebP)、Excel(.xlsx/.xls)、Word(.docx/.doc)、HWP(.hwp/.hwpx)。',
  sd_web_fetch:'从URL获取网页文本。用于阅读新闻、文档等。',
  sd_web_browse:'通过Agent Salad扩展控制您真实的Chrome浏览器。处理Web Fetch无法完成的任务。<b style="color:var(--red)">\\u26A0 会打开您的实际浏览器。</b>仅供自己或信任的人使用。',
  sd_bash:'在服务器上运行Shell命令。用于脚本执行、安装包等。<b style="color:var(--red)">⚠ 可访问整个设备。</b>仅供自己或信任的人使用。',
  sd_terminal:'无超时限制运行命令。用于编程代理（Claude Code、OpenCode）、构建等长时间任务。<b style="color:var(--red)">⚠ 可访问整个设备，无时间限制。</b>仅供自己或信任的人使用。',
  ...
  linkAccount:'',disconnect:'',newsLabel:'',
  commKakao:'韩国用户社区 (KakaoTalk)',commDiscord:'Discord 社区 (全球)',commBugReport:'报告错误',
},
};
...
function setTz(tz){
  fetch('/api/timezone',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({timezone:tz})})
    .then(r=>r.json()).then(d=>{const s=$('tzSelect');if(s)s.value=d.timezone==='auto'?'auto':d.timezone;updateTzLabels();}).catch(()=>{});
}
function tzNow(tz){try{return new Date().toLocaleTimeString('en-GB',{timeZone:tz,hour:'2-digit',minute:'2-digit',hour12:false})}catch(e){return ''}}
function updateTzLabels(){
  const s=$('tzSelect');if(!s)return;
  for(const o of s.options){
    if(o.value==='auto'){
      const sys=Intl.DateTimeFormat().resolvedOptions().timeZone;
      o.textContent='🕐 Auto ('+sys+' '+tzNow(sys)+')';
    }else{
      const short=o.value.split('/').pop().replace(/_/g,' ');
      o.textContent=short+' '+tzNow(o.value);
    }
  }
}
function loadTz(){
  updateTzLabels();
  fetch('/api/timezone').then(r=>r.json()).then(d=>{
    const s=$('tzSelect');if(!s)return;
    if(d.source==='db'){s.value=d.timezone;}else{s.value='auto';}
    updateTzLabels();
  }).catch(()=>{});
}
setInterval(updateTzLabels,60000);
function applyLang(){
  ...
  const m={secActive:'activeServices',secSaladDesc:'saladDesc',useCaseLink:'useCaseLink',secCreate:'createServiceDrag',secCreateDesc:'createServiceDesc',
    ...
    secBrowserExt:'browserExtension',secGoogle:'googleIntegration',secSkillsAll:'allSkills',
    ...
    commKakao:'commKakao',commDiscord:'commDiscord',commBugReport:'commBugReport'};
...
function openUnifiedEveryoneDetail(){
  ...
  panel.innerHTML=
    ...
    '<div class="d-field"><label>'+t('platform')+'</label><div class="d-val">'+['Telegram','Discord','Slack'].concat((D.channelPlugins||[]).map(function(cp){return cp.name})).join(' / ')+'</div></div>'+
...
function getProviderDocs(){return{
  ...
  moonshot:{url:'https://platform.moonshot.ai/docs/pricing/chat',keyUrl:'https://platform.moonshot.ai/console/api-keys',name:'Moonshot (Kimi)',hint:t('provHintMoonshot')},
  glm:{url:'https://docs.bigmodel.cn/cn/guide/start/model-overview',keyUrl:'https://bigmodel.cn/usercenter/proj-mgmt/apikeys',name:'GLM (Zhipu AI)',hint:t('provHintGlm')},
}}
function updateModelGuide(){
  const prov=$('mAgProv')?.value||'anthropic';
  const provObj=D.providers.find(p=>p.id===prov);
  const wrap=$('mAgModelWrap');
  const keyLink=$('mAgProvKeyLink');

  if(provObj&&provObj.has_model_lifecycle){
    var pk=provObj._provider_key;
    var ms=_pluginModelStatus[pk];
    var mountedName=ms&&ms.length>0?ms[0].id:'';
    if(wrap) wrap.innerHTML='<div class="d-field"><label>'+t('model')+'</label>'+
      (mountedName
        ?'<div style="padding:8px 12px;background:var(--green-s);border:1px solid var(--green);border-radius:6px;font-size:.85rem"><span style="color:var(--green)">●</span> '+esc(mountedName)+'</div>'
        :'<div style="padding:8px 12px;background:var(--s1);border:1px solid var(--border);border-radius:6px;font-size:.82rem;color:var(--orange)">'+t('pluginMountFirst')+'</div>')+
      '<input type="hidden" id="mAgModel" value="'+esc(mountedName)+'"></div>';
    if(keyLink)keyLink.innerHTML='';
    return;
  }

  // Restore normal model field if switching from local to another provider
  const inp=$('mAgModel');
  if(!inp||inp.type==='hidden'){
    if(wrap){
      const doc=getProviderDocs()[prov]||{url:'#',keyUrl:'#',name:prov,hint:'model-name'};
      wrap.innerHTML='<div class="d-field"><label>'+t('model')+'</label><div class="field-hint">'+t('modelInputHint')+'</div><input id="mAgModel" placeholder="'+doc.hint+'"><div id="mAgModelGuide" class="model-guide"></div></div>';
    }
  }
  const inp2=$('mAgModel');
  ...
  if(inp2)inp2.placeholder=doc.hint;
  if(keyLink)keyLink.innerHTML=
...
function openAddAgentModal(){
  ...
  const skillToggles=SKILL_IDS.map(k=>{
    ...
    var row='<div class="skill-toggle"><input type="checkbox" id="mSk_'+k+'"'+(on?' checked':'')+'><label for="mSk_'+k+'">'+skillLabel(k)+(skillDesc(k)?'<div class="sk-desc">'+skillDesc(k)+'</div>':'')+'</label><span class="cat">'+skillCat(k)+'</span></div>';
    if(k==='web_browse') row+=browserExtInlineBox();
    return row;
  }).join('');
  ...
  panel.innerHTML=
    ...
    '</div></div>'+
    '<div id="mAgModelWrap"><div class="d-field"><label>'+t('model')+'</label><div class="field-hint">'+t('modelInputHint')+'</div><input id="mAgModel" placeholder="claude-sonnet-4-6"><div id="mAgModelGuide" class="model-guide"></div></div></div>'+
...
function isBuiltinChType(t){return ['telegram','discord','slack'].indexOf(t)!==-1}
function updateChTypeFields(){
  var type=$('mChType').value;
  var isPlugin=!isBuiltinChType(type);
  $('mChTokenHint').innerHTML=chTypeHintFor(type);
  var tokenField=$('mChToken');if(tokenField)tokenField.parentNode.style.display=isPlugin?'none':'';
  ...
  if(tokenField)tokenField.placeholder=ph;
  var guideEl=$('mChGuide');
  var cfgEl=$('mChPluginCfg');
  var plugin=isPlugin?(D.channelPlugins||[]).find(function(cp){return cp.type===type}):null;
  if(guideEl){
    guideEl.innerHTML=(plugin&&plugin.setup_steps)?renderSetupGuide(plugin.setup_steps,plugin._folder_name||plugin.type):'';
  }
  if(cfgEl){
    var sch=plugin&&plugin.config_schema?plugin.config_schema:null;
    cfgEl.innerHTML=sch?renderPluginConfigFields(sch,{},'mChCfg_'):'';
    if(sch)applyCfgConditions(cfgEl,'mChCfg_');
  }
}
function openAddChannelModal(){
  ...
  panel.innerHTML=
    ...
    '<div class="d-field"><label>'+t('platform')+'</label><div class="field-hint">'+t('channelTypeHint')+'</div>'+
      '<select id="mChType" onchange="updateChTypeFields()"><option value="telegram">Telegram</option><option value="discord">Discord</option><option value="slack">Slack</option>'+((D.channelPlugins||[]).map(function(cp){return '<option value="'+esc(cp.type)+'">'+esc(cp.name)+' (plugin)</option>'}).join(''))+'</select></div>'+
    ...
    '<div class="d-field" id="mChAppTokenRow" style="display:none"><label>'+t('appToken')+'</label><div class="field-hint">'+t('appTokenHint')+'</div><input id="mChAppToken" placeholder="Slack app token" style="font-family:var(--mono);font-size:.72rem"></div>'+
    '<div id="mChPluginCfg"></div>'+
    '<div id="mChGuide"></div>'+
    '<div class="d-actions">'+
...
async function submitAddChannel(){
  ...
  var isPlugin=!isBuiltinChType(type);
  var botToken=isPlugin?'':($('mChToken').value.trim());
  if(!isPlugin&&!botToken)return;
  var config=isPlugin?{}:{botToken};
  if(type==='slack'){
  ...
  if(isPlugin){
    var plugin=(D.channelPlugins||[]).find(function(cp){return cp.type===type});
    var sch=plugin&&plugin.config_schema?plugin.config_schema:null;
    if(sch){
      Object.keys(sch).forEach(function(key){
        var el=$('mChCfg_'+key);
        if(!el)return;
        if(sch[key].type==='boolean')config[key]=el.checked;
        else if(sch[key].type==='number')config[key]=el.value===''?null:Number(el.value);
  ...
  if(isPlugin&&pr.success){
    load();
    setTimeout(function(){openChannelDetail(r.id)},300);
    return;
  }
  closeDetail();load();
}
...
function openAddTargetModal(){
  ...
  panel.innerHTML=
    ...
    '<div class="d-field"><label>'+t('platform')+'</label><div class="field-hint">'+t('platformSelectHint')+'</div><select id="mTgPlat" onchange="updateTgPlatFields()"><option value="telegram">Telegram</option><option value="discord">Discord</option><option value="slack">Slack</option>'+((D.channelPlugins||[]).map(function(cp){return '<option value="'+esc(cp.type)+'">'+esc(cp.name)+' (plugin)</option>'}).join(''))+'</select></div>'+
...
let _detailDownTarget=null;
$('detailBg').addEventListener('mousedown',e=>{_detailDownTarget=e.target});
$('detailBg').addEventListener('mouseup',e=>{if(e.target===$('detailBg')&&_detailDownTarget===$('detailBg'))closeDetail();_detailDownTarget=null});
...
function exportChats(serviceId,isTemplate){
  var q=isTemplate?'?children=1':'';
  var a=document.createElement('a');
  a.href='/api/services/'+encodeURIComponent(serviceId)+'/conversations/export'+q;
  a.download='';
  document.body.appendChild(a);a.click();document.body.removeChild(a);
}
document.addEventListener('keydown',e=>{if(e.key==='Escape'&&$('detailBg').classList.contains('show'))closeDetail()});

const SKILL_IDS=['file_read','file_write','file_list','file_upload','web_fetch','web_browse','bash','terminal','google_gmail','google_calendar','google_drive','cron'];
function skillLabel(id){return t('sl_'+id)||id}
function skillCat(id){const catMap={file_read:'file',file_write:'file',file_list:'file',file_upload:'file',web_fetch:'web',web_browse:'web',bash:'system',terminal:'system',google_gmail:'google',google_calendar:'google',google_drive:'google',cron:'automation'};return t('sc_'+(catMap[id]||id))||id}
...
function openAgentDetail(id){
  ...
  const skillToggles=SKILL_IDS.map(k=>{
    ...
    var row='<div class="skill-toggle"><input type="checkbox" id="dSk_'+k+'"'+checked+'><label for="dSk_'+k+'">'+skillLabel(k)+(skillDesc(k)?'<div class="sk-desc">'+skillDesc(k)+'</div>':'')+'</label><span class="cat">'+skillCat(k)+'</span></div>';
    if(k==='web_browse'&&sk[k]) row+=browserExtInlineBox();
    return row;
  }).join('');
  ...
  panel.innerHTML=
    ...
    '<div class="d-field"><label>'+t('provider')+'</label><select id="dAgProv" onchange="onDAgProvChange()">'+provs.map(p=>'<option value="'+esc(p.id)+'"'+(p.id===ag.provider_id?' selected':'')+'>'+esc(p.name)+'</option>').join('')+'</select></div>'+
    '<div id="dAgModelWrap">'+(function(){
      var mlProv=provs.find(function(p){return p.id===ag.provider_id&&p.has_model_lifecycle});
      if(mlProv){
        var pk=mlProv._provider_key;var ms=_pluginModelStatus[pk];
        var mn=ms&&ms.length>0?ms[0].id:'';
        return '<div class="d-field"><label>'+t('model')+'</label>'+
          (mn?'<div style="padding:8px 12px;background:var(--green-s);border:1px solid var(--green);border-radius:6px;font-size:.85rem"><span style="color:var(--green)">●</span> '+esc(mn)+'</div>'
          :'<div style="padding:8px 12px;background:var(--s1);border:1px solid var(--border);border-radius:6px;font-size:.82rem;color:var(--orange)">'+t('pluginMountFirst')+'</div>')+
          '<input type="hidden" id="dAgModel" value="'+esc(mn)+'"></div>';
      }
      return '<div class="d-field"><label>'+t('model')+'</label><input id="dAgModel" value="'+esc(ag.model).replace(/"/g,'&quot;')+'"></div>';
    })()+'</div>'+
...
function onDAgProvChange(){
  var provId=$('dAgProv').value;
  var prov=D.providers.find(function(p){return p.id===provId});
  var wrap=$('dAgModelWrap');
  if(!wrap)return;
  if(prov&&prov.has_model_lifecycle){
    var pk=prov._provider_key;var ms=_pluginModelStatus[pk];
    var mn=ms&&ms.length>0?ms[0].id:'';
    wrap.innerHTML='<div class="d-field"><label>'+t('model')+'</label>'+
      (mn?'<div style="padding:8px 12px;background:var(--green-s);border:1px solid var(--green);border-radius:6px;font-size:.85rem"><span style="color:var(--green)">●</span> '+esc(mn)+'</div>'
      :'<div style="padding:8px 12px;background:var(--s1);border:1px solid var(--border);border-radius:6px;font-size:.82rem;color:var(--orange)">'+t('pluginMountFirst')+'</div>')+
      '<input type="hidden" id="dAgModel" value="'+esc(mn)+'"></div>';
    return;
  ...
  var inp=$('dAgModel');
  if(!inp||inp.type==='hidden'){
    wrap.innerHTML='<div class="d-field"><label>'+t('model')+'</label><input id="dAgModel" placeholder="model-name"></div>';
  }
}
...
function renderSetupGuide(steps,folderName){
  if(!steps||!steps.length)return '';
  var id='setupGuide_'+(folderName||'g');
  var html='<div style="margin:12px 0">';
  html+='<div onclick="var el=$(\\'' +id+ '\\');el.style.display=el.style.display===\\'none\\'?\\'block\\':\\'none\\';this.querySelector(\\'span\\').textContent=el.style.display===\\'none\\'?t(\\'setupGuide\\')+\\' ▸\\':t(\\'setupGuideCollapse\\')+\\' ▾\\'" style="cursor:pointer;padding:8px 12px;background:var(--s1);border:1px solid var(--border);border-radius:8px;font-size:.82rem;font-weight:600;color:var(--accent)"><span>'+t('setupGuide')+' ▸</span></div>';
  html+='<div id="'+id+'" style="display:none;margin-top:8px;border:1px solid var(--border);border-radius:8px;overflow:hidden">';
  steps.forEach(function(step,i){
    var body=(cfgL(step,'body')||'').replace(/\\n/g,'<br>').replace(/ {2}/g,'&nbsp;&nbsp;');
    html+='<div style="padding:12px 16px;border-bottom:1px solid var(--s1)">';
    html+='<div style="font-weight:600;font-size:.84rem;margin-bottom:6px">'+(i+1)+'. '+esc(cfgL(step,'title'))+'</div>';
    html+='<div style="font-size:.78rem;color:var(--t2);line-height:1.6">'+body+'</div>';
    if(step.image&&folderName){
      html+='<img src="/api/dressing-assets/channel/'+esc(folderName)+'/'+esc(step.image)+'" style="max-width:100%;border-radius:6px;margin-top:8px;border:1px solid var(--border)" onerror="this.style.display=\\'none\\'">';
    }
    html+='</div>';
  });
  html+='</div></div>';
...
function cfgL(f,field){return f[field+'_'+_lang]||f[field]||''}
function cfgLArr(f,field){return f[field+'_'+_lang]||f[field]||[]}
function renderFieldLink(f,desc){
  var url=f.link;
  if(!url)return desc?'<div class="field-hint">'+esc(desc)+'</div>':'';
  var lbl=cfgL(f,'link_label')||url;
  return '<div style="margin-top:8px;padding:10px 14px;background:var(--s1);border:1px solid var(--border);border-radius:8px;font-size:.82rem;line-height:1.5">'+(desc?'<div style="color:var(--t2)">'+esc(desc)+'</div>':'')+
    '<div'+(desc?' style="margin-top:6px"':'')+'><a href="'+esc(url)+'" target="_blank" rel="noopener" style="color:var(--indigo);text-decoration:none;font-weight:600">'+esc(lbl)+' \\u2197</a></div></div>';
}
function linkifyText(raw){
  var re=/(https?:\\/\\/[^\\s→)]+)/g;var result='';var last=0;var m;
  while((m=re.exec(raw))!==null){if(m.index>last)result+=esc(raw.slice(last,m.index));result+='<a href="'+esc(m[1])+'" target="_blank" rel="noopener" style="color:var(--indigo);text-decoration:none;font-weight:500">'+esc(m[1])+'</a>';last=re.lastIndex;}
  if(last<raw.length)result+=esc(raw.slice(last));return result||esc(raw);
}
function renderPluginConfigFields(schema,cfg,idPrefix){
  if(!schema)return '';
  var pfx=idPrefix||'dChCfg_';
  var html='';
  var keys=Object.keys(schema);
  keys.forEach(function(key){
    var f=schema[key];
    var val=cfg[key]!=null?cfg[key]:(f.default!=null?f.default:'');
    var label=cfgL(f,'label')||key;
    var desc=cfgL(f,'description');
    var help=cfgL(f,'help');
    var sw=f.show_when;
    var swAttr=sw?(' data-show-when-key="'+esc(Object.keys(sw)[0])+'" data-show-when-val="'+esc(String(Object.values(sw)[0]))+'"'):'';
    if(f.type==='boolean'){
      html+='<div class="d-field cfg-cond"'+swAttr+'><label class="check-label"><input type="checkbox" id="'+pfx+esc(key)+'"'+(val?' checked':'')+' style="width:16px;height:16px;accent-color:var(--indigo);flex-shrink:0">'+esc(label)+'</label>';
      if(f.link){html+=renderFieldLink(f,desc);}
      else{if(desc)html+='<div class="field-hint" style="margin-left:24px">'+esc(desc)+'</div>';}
    } else if(f.type==='select'){
      var opts=cfgLArr(f,'options');
      html+='<div class="d-field cfg-cond"'+swAttr+'><label>'+esc(label)+'</label>';
      html+='<select id="'+pfx+esc(key)+'" style="font-size:.78rem">';
      opts.forEach(function(o){html+='<option value="'+esc(o.value)+'"'+(String(val)===String(o.value)?' selected':'')+'>'+esc(o.label)+'</option>'});
      html+='</select>';
      html+=renderFieldLink(f,desc);
    } else {
      html+='<div class="d-field cfg-cond"'+swAttr+'><label>'+esc(label)+'</label>';
      var ph=cfgL(f,'placeholder')||f.default||'';
      if(f.type==='number'){
        html+='<input type="number" id="'+pfx+esc(key)+'" value="'+esc(String(val??''))+'" placeholder="'+esc(String(ph))+'" style="width:100%;padding:6px 10px;border:1px solid var(--border);border-radius:6px;font-size:.78rem">';
      } else {
        html+='<input id="'+pfx+esc(key)+'" value="'+esc(String(val??'')).replace(/"/g,'&quot;')+'" placeholder="'+esc(String(ph))+'" style="font-size:.78rem">';
      }
      html+=renderFieldLink(f,desc);
    }
    if(help){
      var ml=f.type==='boolean'?' style="margin-left:24px"':'';
      html+='<details'+ml+'><summary style="font-size:.7rem;color:var(--t3);cursor:pointer;user-select:none;list-style:none;display:inline-flex;align-items:center;gap:3px"><svg width="12" height="12" viewBox="0 0 16 16" fill="none" style="opacity:.55"><circle cx="8" cy="8" r="7" stroke="currentColor" stroke-width="1.4"/><path d="M8 7v4M8 5v.5" stroke="currentColor" stroke-width="1.4" stroke-linecap="round"/></svg>'+t('configHelpMore')+'</summary><div style="margin-top:4px;padding:8px 10px;background:var(--s1);border-radius:6px;font-size:.72rem;color:var(--t2);line-height:1.6;white-space:pre-wrap">'+linkifyText(help)+'</div></details>';
    }
    html+='</div>';
  });
  return html;
}
function applyCfgConditions(container,pfx){
  if(!container)return;
  var selects=container.querySelectorAll('select[id^="'+pfx+'"]');
  function upd(){
    var conds=container.querySelectorAll('.cfg-cond[data-show-when-key]');
    conds.forEach(function(el){
      var k=el.getAttribute('data-show-when-key');
      var v=el.getAttribute('data-show-when-val');
      var sel=$(pfx+k);
      if(!sel){el.style.display='';return}
      var cur=sel.tagName==='SELECT'?sel.value:(sel.type==='checkbox'?String(sel.checked):sel.value);
      el.style.display=(cur===v)?'':'none';
    });
  }
  selects.forEach(function(s){s.addEventListener('change',upd)});
  upd();
}
function openChannelDetail(id){
  const ch=D.managedChannels.find(c=>c.id===id);if(!ch)return;
  const paired=ch.pairing_status==='paired';
  const isPlugin=!isBuiltinChType(ch.type);
  const plugin=isPlugin?(D.channelPlugins||[]).find(function(cp){return cp.type===ch.type}):null;
  const schema=plugin&&plugin.config_schema?plugin.config_schema:null;
  var cfg={};try{cfg=JSON.parse(ch.config_json||'{}')}catch{}

  let configDisplay='';
  if(!isPlugin){
    try{if(cfg.botUsername)configDisplay='@'+cfg.botUsername;else if(cfg.botName)configDisplay=cfg.botName+(cfg.teamName?' ('+cfg.teamName+')':'');else if(cfg.botToken)configDisplay=cfg.botToken.slice(0,8)+'...'}catch{}
  }

  var webhookHtml='';
  if(isPlugin&&paired&&schema){
    var hasWebhook=false;var whBase='';
    Object.keys(schema).forEach(function(k){
      if(!schema[k].generates_webhook)return;
      hasWebhook=true;
      if(whBase)return;
      var sw=schema[k].show_when;
      if(sw){var sk=Object.keys(sw)[0];if(String(cfg[sk]||'')!==String(sw[sk]))return}
      var v=String(cfg[k]||'');if(!v)return;
      whBase=(schema[k].webhook_prefix||'')+v.replace(/\\\/+$/,'');
    });
    if(hasWebhook){
      var whUrl=whBase?whBase+'/webhook/'+ch.id:'';
      webhookHtml='<div class="d-field"><label>Webhook URL</label>'+
        (whUrl?'<div class="d-val" style="font-family:var(--mono);font-size:.72rem;word-break:break-all;user-select:all">'+esc(whUrl)+'</div>':'<div class="d-val" style="color:var(--red);font-size:.78rem">⚠ '+t('webhookNeedUrl')+'</div>')+
        '</div>';
    }
  }
  ...
  panel.innerHTML=
    ...
    (isPlugin&&schema?renderPluginConfigFields(schema,cfg):'')+
    webhookHtml+
    (plugin&&plugin.setup_steps?renderSetupGuide(plugin.setup_steps,plugin._folder_name||plugin.type):'')+
    '<div class="d-actions">'+
      '<button class="btn btn-p" onclick="saveChannelDetail(\\''+ch.id+'\\','+(isPlugin&&schema?'true':'false')+')">'+t('save')+'</button>'+
      '<button class="btn btn-g" onclick="closeDetail()">'+t('cancel')+'</button>'+
      '<button class="btn btn-d" style="margin-left:auto" onclick="delCh(\\''+ch.id+'\\');closeDetail()">'+t('delete')+'</button>'+
    '</div>'+
    '';
  $('detailBg').classList.add('show');
  if(isPlugin&&schema)applyCfgConditions(panel,'dChCfg_');
}

async function saveChannelDetail(id,hasPluginConfig){
  const autoEl=$('dChAutoSession');
  const autoSession=autoEl?autoEl.checked?1:0:undefined;
  var updates={name:$('dChName').value.trim(),autoSession:autoSession};
  if(hasPluginConfig){
    var ch=D.managedChannels.find(c=>c.id===id);
    var plugin=ch?(D.channelPlugins||[]).find(cp=>cp.type===ch.type):null;
    var schema=plugin&&plugin.config_schema?plugin.config_schema:null;
    if(schema){
      var pluginCfg={};
      try{pluginCfg=JSON.parse(ch.config_json||'{}')}catch{}
      Object.keys(schema).forEach(function(key){
        var el=$('dChCfg_'+key);
        if(!el)return;
        if(schema[key].type==='boolean')pluginCfg[key]=el.checked;
        else if(schema[key].type==='number')pluginCfg[key]=el.value===''?null:Number(el.value);
        else pluginCfg[key]=el.value;
      });
      updates.config=pluginCfg;
    }
  }
  await api('/api/channels/'+id,'PUT',updates);
  closeDetail();load();
}
...
async function load(){
  ...
  var mlProvs=(D.providers||[]).filter(function(p){return p.has_model_lifecycle});
  for(var i=0;i<mlProvs.length;i++){
    try{
      var st=await api('/api/plugins/'+encodeURIComponent(mlProvs[i]._provider_key)+'/status');
      _pluginModelStatus[mlProvs[i]._provider_key]=st.models||[];
    }catch{_pluginModelStatus[mlProvs[i]._provider_key]=null}
  }
  renderServices();
  renderBlocks();
  renderProviders();
...
function renderServices(){
  ...
  el.innerHTML=groupOrder.map(key=>{
    ...
    const cronCards=allLinkedCrons.map(sc=>{
      ...
      const ttl=cronTimeLeft(sc);
      return '<div class="cron-card" title="'+esc(ttl)+'" onclick="openCronDetail(\\''+sc.cron_id+'\\')">'+
        '<span class="cc-name">'+esc(cj?.name||'?')+'</span>'+
        '<span class="cc-schedule">'+schedLabel+'</span>'+
        (ttl?'<span class="cc-ttl">'+esc(ttl)+'</span>':'')+
    ...
    if(templateSvc){
      ...
      const childCards=concreteSvcs.map(s=>{
        ...
        return '<div class="tg-card'+(s.status==='paused'?' paused':'')+'">'+
          ...
          '<button class="btn btn-g btn-sm" style="position:absolute;top:6px;right:28px;font-size:.65rem;padding:2px 6px" onclick="event.stopPropagation();exportChats(\\''+s.id+'\\',false)">💬</button>'+
      ...
      return '<div class="svc-wrap" '+dropAttr+'>'+
        ...
        '<button class="btn btn-g btn-sm" onclick="event.stopPropagation();exportChats(\\''+templateSvc.id+'\\',true)">💬</button>'+
        ...
        '</div></div>'+
        '<div class="svc-targets-row">'+
          '<span class="svc-targets-label" style="display:flex;align-items:center;gap:6px">'+t('target')+
            (concreteSvcs.length?(' <span style="font-size:.68rem;color:var(--t3);font-weight:400">('+concreteSvcs.length+')</span>')+
            ' <button class="btn btn-g btn-sm" style="font-size:.6rem;padding:1px 6px" onclick="event.stopPropagation();var el=this.closest(\\\'.svc-targets-row\\\').querySelector(\\\'.svc-children\\\');if(el.style.display===\\\'none\\\'){el.style.display=\\\'\\\';this.textContent=\\\'▲\\\'}else{el.style.display=\\\'none\\\';this.textContent=\\\'▼\\\'}">▼</button>':'')+
          '</span>'+
          '<div class="svc-children" style="display:none">'+childCards+'</div>'+
        '</div>'+
    ...
    if(svcs.length===1){
      ...
      return '<div class="svc-wrap" '+dropAttr+'>'+
        ...
        '<button class="btn btn-g btn-sm" onclick="event.stopPropagation();exportChats(\\''+s.id+'\\',false)">💬</button>'+
...
function cronTimeLeft(sc){
  if(!sc||!sc.next_run)return '';
  var diff=new Date(sc.next_run).getTime()-Date.now();
  if(diff<=0)return '';
...
function renderBlocks(){
  ...
  const unifiedEveryoneBlock=(function(){
    ...
    const meta=['Telegram','Discord','Slack'].concat((D.channelPlugins||[]).map(function(cp){return cp.name})).join(' / ')+' TEMPLATE';
  ...
  $('crBlocks').innerHTML=(D.cronJobs||[]).filter(function(cr){return isCronVisible(cr.id)}).map(cr=>{
    ...
    const sc0=(D.serviceCrons||[]).find(sc=>sc.cron_id===cr.id);
    const ttl=cronTimeLeft(sc0);
    return '<div class="blk cr" draggable="true" title="'+esc(ttl)+'" ondragstart="dragMoved=false;onDragStart(event,\\'cron\\',\\''+cr.id+'\\',\\''+esc(cr.name).replace(/'/g,"\\\\&#39;")+'\\');return true" ondragend="onDragEnd(event)" onmousemove="if(event.buttons)dragMoved=true"'+
      ...
      '<div class="blk-meta">'+sBadge+(cr.notify?' \\uD83D\\uDD14':'')+(function(){try{const h=JSON.parse(cr.skill_hint||'[]');return h.length?' \\uD83D\\uDEE0'+h.length:''}catch{return ''}}())+(ttl?' <span class="blk-ttl">'+esc(ttl)+'</span>':'')+'</div>'+
      '</div>'+
...
function renderProviders(){
  ...
  const builtinProvs=D.providers.filter(function(p){return !p._plugin});
  const pluginProvs=D.providers.filter(function(p){return p._plugin});
  let html=builtinProvs.map(p=>{
  ...
  if(pluginProvs.length){
    html+='<div style="margin:14px 0 8px;font-size:.72rem;font-weight:700;color:var(--t3);text-transform:uppercase;letter-spacing:.04em">Plugin Providers</div>';
    html+=pluginProvs.map(function(p){
      return '<div class="prov-card" style="border-left:3px solid #1565c0">'+
        '<div class="prov-header">'+
          '<div class="prov-name">'+esc(p.name)+'</div>'+
          '<span class="prov-badge set" style="background:#e3f2fd;color:#1565c0">PLUGIN</span>'+
        '</div>'+
        '<div style="font-size:.78rem;color:var(--t2);padding:4px 0">provider_key: <code>'+esc(p._provider_key||'')+'</code></div>'+
      '</div>'
    }).join('');
  }
  $('provRows').innerHTML=html;
}
...
async function attachCronToServices(svcIds){
  ...
  if(!cj)return;
  if(svcIds.length>0){
    const svc0=(D.services||[]).find(s=>s.id===svcIds[0]);
    const ch0=svc0&&(D.managedChannels||[]).find(c=>c.id===svc0.channel_id);
    if(ch0&&ch0.supports_outbound===false){showAlert(t('cronNoOutbound'),'⚠️');return}
  }
...
// ── Tab Navigation ──
var _currentTab = 'services';
function switchTab(tab) {
  _currentTab = tab;
  document.querySelectorAll('.tab-btn').forEach(b => b.classList.toggle('active', b.dataset.tab === tab));
  document.querySelectorAll('.tab-panel').forEach(p => p.classList.toggle('active', p.id === 'tab-' + tab));
  var isMain = tab === 'services';
  var cb = document.querySelector('.community-block');
  if (cb) cb.style.display = isMain ? '' : 'none';
...
async function loadAgentsTab() {
  ...
  renderAgentList();
  if (_selectedAgentId && _selectedAgentId.indexOf('__plugin:') === 0) renderPluginModelPanel(_selectedAgentId.slice(9));
  else if (_selectedAgentId) renderAgentDetailPanel(_selectedAgentId);
}

var _pluginModelStatus = {};
...
function renderAgentList() {
  ...
  var pluginCards = '';
  var mlProvs = (D.providers || []).filter(function(p) { return p.has_model_lifecycle; });
  mlProvs.forEach(function(p) {
    var pk = p._provider_key;
    var sel = _selectedAgentId === '__plugin:' + pk ? ' selected' : '';
    var ms = _pluginModelStatus[pk];
    var statusText = ms && ms.length > 0
      ? '<span style="color:var(--green)">● </span>' + esc(ms[0].id)
      : '<span style="color:var(--t3)">○ </span>' + t('pluginNoModel');
    pluginCards += '<div class="ag-card' + sel + '" onclick="selectPluginModelTab(\\'' + esc(pk) + '\\')" style="border-left:3px solid var(--accent)">' +
      '<div class="ag-card-name">' + esc(p._display_name || p.name) + '</div>' +
      '<div class="ag-card-meta">' + statusText + '</div></div>';
  });

  if (!D.agentProfiles || D.agentProfiles.length === 0) {
    el.innerHTML = pluginCards + '<div class="agents-empty">' + esc(t('noAgents')) + '</div>';
    return;
  }
  el.innerHTML = pluginCards + D.agentProfiles.map(ag => {
...
function selectPluginModelTab(providerKey) {
  _selectedAgentId = '__plugin:' + providerKey;
  document.querySelectorAll('.ag-card').forEach(function(c) { c.classList.remove('selected'); });
  var mlProvs = (D.providers || []).filter(function(p) { return p.has_model_lifecycle; });
  var idx = mlProvs.findIndex(function(p) { return p._provider_key === providerKey; });
  var cards = document.querySelectorAll('.ag-card');
  if (idx >= 0 && cards[idx]) cards[idx].classList.add('selected');
  renderPluginModelPanel(providerKey);
}

var _pluginBrowseDir = '';
var _currentPluginKey = '';
async function renderPluginModelPanel(providerKey) {
  var panel = $('agentsDetail');
  if (!panel) return;
  _currentPluginKey = providerKey;
  var prov = (D.providers || []).find(function(p) { return p._provider_key === providerKey; });
  var displayName = (prov && prov._display_name) || providerKey;

  var ms;
  try {
    var st = await api('/api/plugins/' + encodeURIComponent(providerKey) + '/status');
    ms = st.models || [];
    _pluginModelStatus[providerKey] = ms;
  } catch { ms = []; _pluginModelStatus[providerKey] = []; }

  var statusHtml = '';
  if (ms && ms.length > 0) {
    var m = ms[0];
    var sizeStr = m.size_bytes ? ' (' + (m.size_bytes / 1073741824).toFixed(1) + ' GB)' : '';
    statusHtml = '<div style="display:flex;align-items:center;gap:8px;padding:12px 16px;background:var(--green-s);border:1px solid var(--green);border-radius:8px">' +
      '<span style="color:var(--green);font-size:1.2rem">●</span>' +
      '<div><div style="font-weight:600;font-size:.88rem">' + esc(m.id) + '</div>' +
      '<div style="font-size:.75rem;color:var(--t3)">loaded' + esc(sizeStr) + '</div></div></div>';
  } else {
    statusHtml = '<div style="padding:12px 16px;background:var(--s1);border:1px solid var(--border);border-radius:8px;color:var(--t3);font-size:.85rem">' +
      '○ ' + t('pluginNoModel') + '</div>';
  }

  var unloadBtn = (ms && ms.length > 0)
    ? '<button class="btn btn-d btn-sm" onclick="pluginUnload()" id="pluginUnloadBtn">⏏ ' + t('pluginModelUnmount') + '</button>'
    : '';

  var cfgHtml = '';
  try {
    var cfgRes = await api('/api/plugins/' + encodeURIComponent(providerKey) + '/config');
    var cfg = cfgRes.config || {};
    var sch = cfgRes.schema || {};
    if (Object.keys(sch).length > 0) {
      var modelMounted = ms && ms.length > 0;
      cfgHtml = '<div class="agent-section"><h4>' + t('pluginSettings') + '</h4>' +
        renderPluginConfigFields(sch, cfg, 'pcfg_') +
        (modelMounted ? '<div style="padding:8px 12px;background:var(--orange-s,#fff8e1);border:1px solid var(--orange,#ffa726);border-radius:6px;font-size:.75rem;color:var(--orange,#e65100);margin-top:8px">⚠ ' + t('configSaveWarnModel') + '</div>' : '') +
        '<button class="btn btn-p btn-sm" onclick="pluginSaveConfig()" id="pluginSaveBtn" style="margin-top:10px">' + t('save') + '</button>' +
        '<span id="pluginSaveOk" style="color:var(--green);font-size:.78rem;margin-left:8px;display:none">' + t('pluginSaved') + '</span></div>';
    }
  } catch {}

  var guideHtml = '';
  if (prov && prov._guide_url) {
    var gt = prov['_guide_text_'+_lang] || prov._guide_text || prov._guide_url;
    guideHtml = '<div class="agent-section" style="margin-top:16px"><a href="' + esc(prov._guide_url) + '" target="_blank" style="font-size:.8rem;color:var(--accent)">' + esc(gt) + ' →</a></div>';
  }

  panel.innerHTML =
    '<h3 style="margin:0 0 16px">' + esc(displayName) + '</h3>' +
    '<div class="agent-section"><h4>' + t('pluginModelStatus') + '</h4>' + statusHtml + '</div>' +
    '<div class="agent-section"><h4>' + t('pluginModelMount') + '</h4>' +
    '<div id="pluginBrowse" style="border:1px solid var(--border);border-radius:8px;overflow:hidden">' +
    '<div style="display:flex;align-items:center;gap:6px;padding:8px 12px;background:var(--s1);border-bottom:1px solid var(--border)">' +
    '<span style="font-size:.78rem;color:var(--t3)" id="pluginBrowsePath">~</span>' +
    '<button class="btn btn-g btn-sm" onclick="pluginBrowseUp()" style="margin-left:auto;padding:2px 8px;font-size:.72rem">↑</button></div>' +
    '<div id="pluginBrowseList" style="max-height:260px;overflow-y:auto"></div></div>' +
    '<div style="display:flex;gap:8px;margin-top:10px;align-items:center">' +
    '<input id="pluginModelPath" placeholder="' + t('pluginModelPath') + '" style="flex:1;padding:8px 12px;border:1px solid var(--border);border-radius:6px;font-size:.82rem">' +
    '<button class="btn btn-p btn-sm" onclick="pluginMount()" id="pluginMountBtn">▶ ' + t('pluginModelMount') + '</button></div>' +
    '<div id="pluginError" style="color:var(--red);font-size:.78rem;margin-top:8px;display:none"></div></div>' +
    (unloadBtn ? '<div class="agent-section"><h4>' + t('pluginModelUnmount') + '</h4>' + unloadBtn + '</div>' : '') +
    cfgHtml + guideHtml;

  applyCfgConditions(panel,'pcfg_');
  pluginBrowseDir('');
}

function escJsStr(s) { return s.replace(/\\\\/g,'\\\\\\\\').replace(/'/g,"\\\\'"); }

async function pluginBrowseDir(dir) {
  try {
    var res = await api('/api/plugins/' + encodeURIComponent(_currentPluginKey) + '/browse?dir=' + encodeURIComponent(dir));
    _pluginBrowseDir = res.dir;
    $('pluginBrowsePath').textContent = res.dir;
    $('pluginBrowseList').innerHTML = res.items.map(function(it) {
      var safePath = escJsStr(it.path);
      if (it.isDir) {
        return '<div onclick="pluginBrowseDir(\\'' + safePath + '\\')" style="padding:6px 12px;cursor:pointer;display:flex;align-items:center;gap:6px;border-bottom:1px solid var(--s1);font-size:.82rem" onmouseover="this.style.background=\\'var(--s1)\\'" onmouseout="this.style.background=\\'\\'"><span>📁</span>' + esc(it.name) + '</div>';
      }
      var sizeStr = it.size ? (it.size / 1073741824).toFixed(2) + ' GB' : '';
      return '<div onclick="pluginSelectFile(\\'' + safePath + '\\')" style="padding:6px 12px;cursor:pointer;display:flex;align-items:center;gap:6px;border-bottom:1px solid var(--s1);font-size:.82rem" onmouseover="this.style.background=\\'var(--cyan-s)\\'" onmouseout="this.style.background=\\'\\'"><span>📄</span>' + esc(it.name) + '<span style="margin-left:auto;color:var(--t3);font-size:.72rem">' + esc(sizeStr) + '</span></div>';
    }).join('') || '<div style="padding:12px;color:var(--t3);font-size:.78rem">' + t('pluginNoFiles') + '</div>';
  } catch (e) {
    $('pluginBrowseList').innerHTML = '<div style="padding:12px;color:var(--red);font-size:.78rem">' + esc(e.message || String(e)) + '</div>';
  }
}

function pluginBrowseUp() {
  if (!_pluginBrowseDir) return;
  var parent = _pluginBrowseDir.replace(/[\\/\\\\][^\\/\\\\]+[\\/\\\\]?$/, '');
  if (!parent) parent = _pluginBrowseDir.match(/^[A-Za-z]:/) ? _pluginBrowseDir.slice(0,3) : '/';
  if (/^[A-Za-z]:$/.test(parent)) parent += '\\\\';
  pluginBrowseDir(parent);
}

function pluginSelectFile(path) {
  $('pluginModelPath').value = path;
}

async function pluginMount() {
  var fpath = $('pluginModelPath').value.trim();
  if (!fpath) return;
  var errEl = $('pluginError');
  errEl.style.display = 'none';
  $('pluginMountBtn').disabled = true;
  $('pluginMountBtn').textContent = '⏳ ...';
  try {
    var res = await api('/api/plugins/' + encodeURIComponent(_currentPluginKey) + '/load', 'POST', { model_path: fpath });
    if (!res.success) throw new Error(res.error || 'Mount failed');
    await renderPluginModelPanel(_currentPluginKey);
    renderAgentList();
  } catch (e) {
    errEl.textContent = e.message || String(e);
    errEl.style.display = 'block';
    $('pluginMountBtn').disabled = false;
    $('pluginMountBtn').textContent = '▶ ' + t('pluginModelMount');
  }
}

async function pluginUnload() {
  var ms = _pluginModelStatus[_currentPluginKey];
  if (!ms || ms.length === 0) return;
  $('pluginUnloadBtn').disabled = true;
  $('pluginUnloadBtn').textContent = '⏳ ...';
  try {
    await api('/api/plugins/' + encodeURIComponent(_currentPluginKey) + '/unload', 'POST', { model_id: ms[0].id });
    await renderPluginModelPanel(_currentPluginKey);
    renderAgentList();
  } catch (e) {
    var errEl = $('pluginError');
    if (errEl) { errEl.textContent = e.message || String(e); errEl.style.display = 'block'; }
    $('pluginUnloadBtn').disabled = false;
    $('pluginUnloadBtn').textContent = '⏏ ' + t('pluginModelUnmount');
  }
}

async function pluginSaveConfig() {
  var btn = $('pluginSaveBtn');
  var ok = $('pluginSaveOk');
  if (btn) btn.disabled = true;
  if (ok) ok.style.display = 'none';
  try {
    var cfgRes = await api('/api/plugins/' + encodeURIComponent(_currentPluginKey) + '/config');
    var sch = cfgRes.schema || {};
    var cfg = {};
    for (var k of Object.keys(sch)) {
      var el = $('pcfg_' + k);
      if (!el) continue;
      if (sch[k].type === 'boolean') cfg[k] = el.checked;
      else if (sch[k].type === 'number') cfg[k] = Number(el.value) || sch[k].default;
      else cfg[k] = el.value;
    }
    await api('/api/plugins/' + encodeURIComponent(_currentPluginKey) + '/config', 'POST', { config: cfg });
    if (ok) { ok.style.display = 'inline'; setTimeout(function() { ok.style.display = 'none'; }, 2000); }
    setTimeout(async function() { await renderPluginModelPanel(_currentPluginKey); renderAgentList(); }, 1500);
  } catch (e) {
...
function selectAgentTab(id) {
  ...
  const offset = (D.providers || []).filter(function(p) { return p.has_model_lifecycle; }).length;
  if (idx >= 0 && cards[idx + offset]) cards[idx + offset].classList.add('selected');
...
async function renderAgentDetailPanel(id) {
  ...
  const skillToggles = SKILL_IDS.map(k => {
    const checked = sk[k] ? ' checked' : '';
    var row = '<div class="skill-toggle"><input type="checkbox" id="atSk_' + k + '"' + checked + ' onchange="selectedAgentSkills[\\'' + k + '\\']=this.checked"><label for="atSk_' + k + '">' + skillLabel(k) + (skillDesc(k) ? '<div class="sk-desc">' + skillDesc(k) + '</div>' : '') + '</label><span class="cat">' + skillCat(k) + '</span></div>';
    if (k === 'web_browse' && sk[k]) row += browserExtInlineBox();
    return row;
  }).join('');
  ...
  panel.innerHTML =
    ...
    '<div class="d-field"><label>' + t('provider') + '</label><select id="atAgProv" onchange="onAtAgProvChange()">' + provs.map(p => '<option value="' + esc(p.id) + '"' + (p.id === ag.provider_id ? ' selected' : '') + '>' + esc(p.name) + '</option>').join('') + '</select></div>' +
    '<div id="atAgModelWrap">' + (function() {
      var mlProv = provs.find(function(p) { return p.id === ag.provider_id && p.has_model_lifecycle; });
      if (mlProv) {
        var pk = mlProv._provider_key; var ms = _pluginModelStatus[pk];
        var mountedName = ms && ms.length > 0 ? ms[0].id : '';
        return '<div class="d-field"><label>' + t('model') + '</label>' +
          (mountedName
            ? '<div style="padding:8px 12px;background:var(--green-s);border:1px solid var(--green);border-radius:6px;font-size:.85rem"><span style="color:var(--green)">●</span> ' + esc(mountedName) + '</div>'
            : '<div style="padding:8px 12px;background:var(--s1);border:1px solid var(--border);border-radius:6px;font-size:.82rem;color:var(--orange)">' + t('pluginMountFirst') + '</div>') +
          '<input type="hidden" id="atAgModel" value="' + esc(mountedName) + '"></div>';
      }
      return '<div class="d-field"><label>' + t('model') + '</label><input id="atAgModel" value="' + esc(ag.model).replace(/"/g, '&quot;') + '"></div>';
    })() + '</div>' +
...
function onAtAgProvChange() {
  const provId = $('atAgProv').value;
  const prov = D.providers.find(p => p.id === provId);
  const wrap = $('atAgModelWrap');
  if (!wrap) return;
  if (prov && prov.has_model_lifecycle) {
    var pk = prov._provider_key; var ms = _pluginModelStatus[pk];
    var mountedName = ms && ms.length > 0 ? ms[0].id : '';
    ...
    wrap.innerHTML = '<div class="d-field"><label>' + t('model') + '</label><input id="atAgModel" value="' + esc(curVal) + '"></div>';
  }
}
...
async function loadSkillsTab() {
  const [bs, cs, gog, brw, installed] = await Promise.all([
    api('/api/builtin-skills'),
    api('/api/custom-skills'),
    api('/api/integrations/google/status'),
    api('/api/integrations/browser/status'),
    api('/api/dressing/installed'),
  ]);
  builtinSkills = bs || [];
  customSkillPool = cs || [];
  _installedPlugins = (installed || []).filter(function(d){return d.type==='agent'||d.type==='channel'});
  renderSkillList();
  if (_selectedSkillId) renderSkillDetailPanel(_selectedSkillId, _selectedSkillType);
  renderBrowserExtStatus(brw);
  renderGogStatus(gog);
}
...
function renderSkillList() {
  ...
  // Installed plugins (agent + channel dressings)
  if (_installedPlugins && _installedPlugins.length) {
    html += '<div style="font-size:.7rem;font-weight:700;color:var(--t3);padding:8px 4px 4px;text-transform:uppercase;letter-spacing:.04em">PLUGINS</div>';
    _installedPlugins.forEach(function(p) {
      var sel = (_selectedSkillType === 'plugin' && _selectedSkillId === p.db_id) ? ' selected' : '';
      var typeBadge = p.type === 'agent'
        ? '<span class="sk-badge" style="background:#e3f2fd;color:#1565c0">PROVIDER</span>'
        : '<span class="sk-badge" style="background:#fce4ec;color:#c62828">CHANNEL</span>';
      var keyLabel = p.provider_key || p.channel_type || '';
      html += '<div class="sk-card' + sel + '" onclick="selectPlugin(\\'' + p.db_id + '\\')">' +
        '<div class="sk-name">' + esc(p.name) + '</div>' +
        '<div class="sk-meta">' + typeBadge + ' <span style="font-size:.72rem;color:var(--t3)">' + esc(keyLabel) + ' v' + esc(p.version) + '</span></div>' +
        '</div>';
    });
  }
...
function selectSkill(id, type) {
  ...
  let idx = -1;
  var pluginOffset = _installedPlugins ? _installedPlugins.length : 0;
  if (type === 'custom') {
    idx = pluginOffset + customSkillPool.findIndex(s => s.id === id);
  } else {
    idx = pluginOffset + customSkillPool.length + builtinSkills.findIndex(s => s.name === id);
  }
...
function selectPlugin(id) {
  _selectedSkillId = id;
  _selectedSkillType = 'plugin';
  document.querySelectorAll('#skillsListItems .sk-card').forEach(c => c.classList.remove('selected'));
  var allCards = document.querySelectorAll('#skillsListItems .sk-card');
  var idx = _installedPlugins.findIndex(function(p){return p.db_id===id});
  if (idx >= 0 && allCards[idx]) allCards[idx].classList.add('selected');
  renderPluginDetailPanel(id);
}

function renderPluginDetailPanel(id) {
  var p = _installedPlugins.find(function(d){return d.db_id===id});
  var el = $('skillDetail');
  if (!el || !p) return;
  var typeLabel = p.type === 'agent' ? 'Provider Plugin' : 'Channel Plugin';
  var keyLabel = p.provider_key || p.channel_type || '-';
  el.innerHTML =
    '<div class="sk-detail-hdr">' +
      '<div class="sk-detail-name">' + esc(p.name) + '</div>' +
      '<span class="sk-badge" style="background:' + (p.type==='agent'?'#e3f2fd':'#fce4ec') +
      ';color:' + (p.type==='agent'?'#1565c0':'#c62828') + '">' + typeLabel + '</span>' +
    '</div>' +
    '<div style="margin:16px 0;display:grid;grid-template-columns:auto 1fr;gap:6px 16px;font-size:.82rem">' +
      '<span style="color:var(--t3)">Key</span><span>' + esc(keyLabel) + '</span>' +
      '<span style="color:var(--t3)">Version</span><span>v' + esc(p.version) + '</span>' +
      '<span style="color:var(--t3)">Installed</span><span>' + esc(p.installed_at||'-') + '</span>' +
      '<span style="color:var(--t3)">Dressing ID</span><span style="font-size:.72rem;color:var(--t3)">' + esc(p.skill_id||'-') + '</span>' +
    '</div>' +
    '<div style="margin-top:16px;display:flex;gap:8px">' +
      '<button class="btn btn-sm" onclick="deletePlugin(\\'' + id + '\\')">Uninstall</button>' +
    '</div>';
}

async function deletePlugin(id) {
  if (!confirm('Uninstall this plugin?')) return;
  await fetch('/api/dressing/plugin', {method:'DELETE', headers:{'Content-Type':'application/json'}, body:JSON.stringify({dressing_id:id})});
...
async function renderSkillDetailPanel(id, type) {
  ...
  if (type === 'builtin') {
    ...
    const isBrowse = s.id === 'web_browse';
    ...
    const browseBox = isBrowse ? browserExtInlineBox() : '';
    panel.innerHTML =
  ...
  // ── Local custom skill: editable inline editor ──
  const isScript = cs.tool_name && cs.tool_name.trim();
...
function browserExtInlineBox() {
  var dlUrl = 'https://github.com/terry-uu/agentsalad/tree/main/extension';
  return '<div style="margin-top:6px;padding:10px 12px;background:var(--s2);border-radius:8px;font-size:.72rem;color:var(--t2);line-height:1.6">' +
    '<div style="font-weight:600;color:var(--t1);margin-bottom:4px;font-size:.74rem">' + esc(t('browserExtension')) + '</div>' +
    '<div style="margin-bottom:6px">' +
      '1. ' + esc(t('browserExtStep2')) + '<br>' +
      '2. ' + esc(t('browserExtStep3')) + '<br>' +
      '3. ' + esc(t('browserExtStep4')) +
    '</div>' +
    '<a href="' + dlUrl + '" target="_blank" class="btn btn-p btn-sm" style="display:inline-flex;align-items:center;gap:5px;text-decoration:none;font-size:.68rem;padding:4px 10px">' +
      ...
      esc(t('browserExtDownload')) +
    '</a>' +
  '</div>';
}

function renderBrowserExtStatus(status) {
  var el = $('browserExtStatus');
  var connected = status?.connected;
  var dlUrl = 'https://github.com/terry-uu/agentsalad/tree/main/extension';
  if (connected) {
    el.innerHTML = '<div style="color:var(--green);font-size:.82rem">\\u2713 '+esc(t('browserExtConnected'))+'</div>';
    return;
  }
  el.innerHTML =
    '<div style="font-size:.82rem">' +
      '<p style="color:var(--t2);margin-bottom:10px">'+esc(t('browserExtNotConnected'))+'</p>' +
      '<div style="background:var(--s2);padding:14px;border-radius:8px;font-size:.74rem;color:var(--t2);line-height:1.7">' +
        '<div style="margin-bottom:10px;line-height:1.5">'+esc(t('browserExtExplain'))+'</div>' +
        '<div style="padding-left:8px;margin-bottom:12px">' +
          '1. '+esc(t('browserExtStep1'))+'<br>' +
          '2. '+esc(t('browserExtStep2'))+'<br>' +
          '3. '+esc(t('browserExtStep3'))+'<br>' +
          '4. '+esc(t('browserExtStep4'))+
        '</div>' +
        '<a href="'+dlUrl+'" target="_blank" class="btn btn-p btn-sm" style="display:inline-flex;align-items:center;gap:6px;text-decoration:none">' +
    ...
    '<div style="font-size:.82rem">' +
      ...
      '<div style="background:var(--s2);padding:14px;border-radius:8px;font-size:.74rem;color:var(--t2);line-height:1.7">' +
        ...
        '<div style="padding-left:8px;margin-bottom:12px">' +
          ...
          '3. <code style="background:var(--s1);padding:1px 5px;border-radius:3px">gog auth add user@example.com</code>' +
        '</div>' +
...
var _bugReportFormUrl='https://github.com/terry-uu/agentsalad/issues/new/choose';
</script>
</body>
</html>`;
}
...
export function startWebUiServer(
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (url.pathname === '/api/overview') {
        const agentPlugins = listInstalledDressings('agent').map((d) => {
          const plugin = loadAgentPlugin(d.provider_key ?? '');
          let meta: Record<string, unknown> = {};
          try {
            const djPath = path.join(
              STORE_DIR,
              'dressings',
              'agent',
              d.folder_name,
            ...
            meta = JSON.parse(fs.readFileSync(djPath, 'utf-8'));
          } catch {}
          return {
            id: `plugin:${d.provider_key}`,
            name: `${d.name} (plugin)`,
            api_key: '***',
            _plugin: true,
            _dressing_id: d.id,
            _provider_key: d.provider_key,
            has_model_lifecycle: !!plugin?.getModelStatus,
            _display_name: (meta.display_name as string) || d.name,
            _guide_url: (meta.guide_url as string) || null,
            _guide_text: (meta.guide_text as string) || null,
            _guide_text_ko: (meta.guide_text_ko as string) || null,
            _guide_text_ja: (meta.guide_text_ja as string) || null,
            _guide_text_zh: (meta.guide_text_zh as string) || null,
            _file_extensions: (meta.file_extensions as string[]) || null,
          };
        });
        const channelPlugins = listInstalledDressings('channel').map((d) => {
          let setupSteps: unknown[] | null = null;
          try {
            ...
            if (fs.existsSync(metaPath)) {
              ...
              if (Array.isArray(m.setup_steps)) setupSteps = m.setup_steps;
            }
          } catch {}
          return {
            type: d.channel_type,
            ...
            _plugin: true,
            _dressing_id: d.id,
            _folder_name: d.folder_name,
            config_schema: d.config_schema ? JSON.parse(d.config_schema) : null,
        ...
        sendJson(res, 200, {
          ...
          managedChannels: context.listManagedChannels().map((ch) => ({
            ...ch,
            supports_outbound: channelSupportsOutbound(ch.type),
          })),
      ...
      if (url.pathname === '/api/channels' && req.method === 'POST') {
        const body = await readJsonBody(req);
        const builtinTypes: string[] = [...BUILTIN_CHANNEL_TYPES];
        const pluginTypes = listInstalledDressings('channel')
          .map((d) => d.channel_type)
          .filter(Boolean) as string[];
        const validTypes = [...builtinTypes, ...pluginTypes];
        const type = validTypes.includes(body.type as string)
      ...
      if (
        ...
        if (!['telegram', 'discord', 'slack'].includes(channelType)) {
          // 플러그인 채널 — 토큰 검증 불필요, 즉시 paired 처리
          // config는 POST /api/channels에서 이미 저장됨 — 여기서 덮어쓰지 않음
          context.updateChannelPairing(channelId, 'paired');
          try {
            await context.connectChannel(channelId);
          } catch {
            // 웹훅 기반 채널은 connect 실패해도 paired 유지
          }
          result = { success: true, botUsername: channelType };
      ...
      if (
        ...
        context.updateManagedChannel(id, {
          ...
          configJson: hasConfig ? JSON.stringify(body.config) : undefined,
        });
        if (hasConfig) {
          try {
            await context.connectChannel(id);
          } catch {}
        }
        sendJson(res, 200, { ok: true });
        return;
      ...
      if (url.pathname === '/api/targets' && req.method === 'POST') {
        ...
        const pluginPlatforms = listInstalledDressings('channel')
          .map((d) => d.channel_type)
          .filter(Boolean) as string[];
        ...
        const platform = validPlatforms.includes(body.platform as string)
      ...
      if (
        ...
        const pluginPlats = listInstalledDressings('channel')
          .map((d) => d.channel_type)
          .filter(Boolean) as string[];
        const validPlats: string[] = [...BUILTIN_CHANNEL_TYPES, ...pluginPlats];
      ...
      if (
        url.pathname.match(/^\/api\/services\/[^/]+\/conversations$/) &&
        req.method === 'GET'
      ) {
        const serviceId = decodeURIComponent(url.pathname.split('/')[3]);
      ...
      if (
        url.pathname.match(/^\/api\/services\/[^/]+\/conversations\/export$/) &&
        req.method === 'GET'
      ) {
        const serviceId = decodeURIComponent(url.pathname.split('/')[3]);
        ...
        const rows: string[] = ['timestamp,target,role,content'];

        if (children) {
          const childSvcs = listConcreteServicesForTemplate(serviceId);
          for (const cs of childSvcs) {
            const msgs = getConversationHistory(cs.id, 1000);
            const tgName = cs.target?.nickname || cs.target_id;
            for (const m of msgs) {
              rows.push(
                [
                  m.timestamp || '',
          ...
          const msgs = getConversationHistory(serviceId, 1000);
          const svc = getServiceById(serviceId);
          const tg = svc ? getTargetById(svc.target_id) : undefined;
          const tgName = tg?.nickname || svc?.target_id || 'unknown';
          for (const m of msgs) {
            rows.push(
              [
                m.timestamp || '',
        ...
        const csv = rows.join('\n');
        const filename = `chat-export-${serviceId.slice(0, 8)}-${new Date().toISOString().slice(0, 10)}.csv`;
        res.writeHead(200, {
      ...
      if (
        ...
        const svcForCron = getServiceById(serviceId);
        if (svcForCron) {
          const ch = getManagedChannelById(svcForCron.channel_id);
          if (ch && !channelSupportsOutbound(ch.type)) {
            sendJson(res, 400, {
              ok: false,
              error: `Channel type "${ch.type}" does not support outbound messages (cron)`,
            });
            return;
          }
        }
      ...
      if (
        url.pathname === '/api/integrations/browser/status' &&
        req.method === 'GET'
      ) {
        sendJson(res, 200, context.getBrowserExtensionStatus());
        return;
      }

      // Timezone
      if (url.pathname === '/api/timezone' && req.method === 'GET') {
        sendJson(res, 200, {
          timezone: getTimezone(),
          source: getTimezoneSource(),
        });
        return;
      }
      if (url.pathname === '/api/timezone' && req.method === 'PUT') {
        const body = await readJsonBody(req);
        const tz = body?.timezone;
        if (!tz || typeof tz !== 'string') {
          sendJson(res, 400, { error: 'timezone required' });
          return;
        }
        if (tz === 'auto') {
          deleteServerSetting('timezone');
          resetTimezone();
          const autoTz = getTimezone();
          for (const cron of listCronJobs()) {
            if (cron.schedule_type !== 'weekly') continue;
            const days = parseScheduleDays(cron.schedule_days);
            const nextRun = computeWeeklyNextRun(
              cron.schedule_time,
              days,
              autoTz,
            );
            if (nextRun) updateNextRunByCronId(cron.id, nextRun);
          }
          sendJson(res, 200, {
            timezone: autoTz,
            source: getTimezoneSource(),
        ...
        try {
          ...
          sendJson(res, 400, { error: `Invalid timezone: ${tz}` });
          return;
        }
        setServerSetting('timezone', tz);
        setTimezone(tz);

        // 기존 weekly 크론의 next_run을 새 타임존으로 즉시 재계산
        let recalculated = 0;
        for (const cron of listCronJobs()) {
          if (cron.schedule_type !== 'weekly') continue;
          const days = parseScheduleDays(cron.schedule_days);
          const nextRun = computeWeeklyNextRun(cron.schedule_time, days, tz);
          if (nextRun) {
            updateNextRunByCronId(cron.id, nextRun);
            recalculated++;
          }
        }

        logger.info(
          { timezone: tz, recalculatedCrons: recalculated },
          'Server timezone changed via Web UI',
        );
        sendJson(res, 200, { timezone: tz, source: 'db' });
        return;
      }
      ...
      // ── 드레싱 정적 파일 서빙 (셋업 가이드 이미지 등) ──
      const assetMatch = url.pathname.match(
        /^\/api\/dressing-assets\/(agent|channel)\/([a-zA-Z0-9_-]+)\/([a-zA-Z0-9_.-]+)$/,
      );
      if (assetMatch && req.method === 'GET') {
        const [, dtype, folder, filename] = assetMatch;
      ...
      // ── 범용 웹훅 수신 (채널 드레싱 플러그인) ──
      // HTTP 요청의 전체 구성을 플러그인에 투명 전달. GET(verification challenge) 포함.
      const webhookMatch = url.pathname.match(/^\/webhook\/([a-zA-Z0-9_-]+)$/);
      if (webhookMatch) {
        const channelId = webhookMatch[1];
        const { loadChannelPlugin } =
          await import('./dressing/plugin-loader.js');
        ...
        const plugin = loadChannelPlugin(channel.type);
        if (!plugin?.handleWebhook) {
          sendJson(res, 404, { error: 'No webhook handler for this channel' });
          return;
        }
        ...
        try {
          const result = plugin.handleWebhook({
            method,
            path: url.pathname,
            query,
            headers,
            body: rawBody,
            channelId,
            onMessage: context.onServiceMessage,
          });
          ...
          logger.error({ err, channelId }, 'Webhook handler error');
          sendJson(res, 500, { error: 'Webhook processing failed' });
        }
        return;
      }

      // ── Generic Plugin API (/api/plugins/:providerKey/:action) ──
      {
        const pluginMatch = url.pathname.match(
          /^\/api\/plugins\/([^/]+)\/([^/]+)$/,
        );
        if (pluginMatch) {
          const [, providerKey, action] = pluginMatch;
          const agentPlugin = loadAgentPlugin(providerKey);
          if (!agentPlugin) {
            sendJson(res, 404, { error: 'Plugin not installed' });
            return;
          }

          if (action === 'status' && req.method === 'GET') {
            try {
              await initAgentPlugin(providerKey);
              const models = agentPlugin.getModelStatus
                ? await agentPlugin.getModelStatus()
                : [];
          ...
          if (action === 'load' && req.method === 'POST') {
            const body = await readJsonBody(req);
            const modelPath = body.model_path as string;
            if (!modelPath) {
              sendJson(res, 400, { error: 'model_path required' });
              return;
            }
            try {
              await initAgentPlugin(providerKey);
              if (!agentPlugin.loadModel) {
                sendJson(res, 501, { error: 'loadModel not supported' });
                return;
              }
              const result = await agentPlugin.loadModel(modelPath);
          ...
          if (action === 'unload' && req.method === 'POST') {
            ...
            if (!modelId) {
              sendJson(res, 400, { error: 'model_id required' });
              return;
            }
            try {
              await initAgentPlugin(providerKey);
              if (!agentPlugin.unloadModel) {
                sendJson(res, 501, { error: 'unloadModel not supported' });
                return;
              }
              const result = await agentPlugin.unloadModel(modelId);
              sendJson(res, result.success ? 200 : 500, result);
            } catch (err) {
              sendJson(res, 500, {
                error: err instanceof Error ? err.message : String(err),
          ...
          if (action === 'browse' && req.method === 'GET') {
            ...
            if (dressing) {
              try {
                ...
                const djMeta = JSON.parse(fs.readFileSync(djPath, 'utf-8'));
                if (Array.isArray(djMeta.file_extensions)) {
                  fileExts = djMeta.file_extensions;
                }
              } catch {}
            }
            const dir = (url.searchParams.get('dir') || homedir()).replace(
              /^~/,
              homedir(),
            );
            try {
              ...
              const items = entries
                .filter(
                  (e) =>
                    ...
                    (e.isDirectory() ||
                      fileExts.length === 0 ||
                      fileExts.some((ext) => e.name.endsWith(ext))),
                )
                .map((e) => ({
                  name: e.name,
                  ...
                  isDir: e.isDirectory(),
                  size: e.isFile()
                    ? statSync(join(resolved, e.name)).size
                    : undefined,
                }))
                .sort((a, b) => {
              ...
              sendJson(res, 200, { dir: resolved, items });
            } catch (err) {
              sendJson(res, 400, {
                error: err instanceof Error ? err.message : String(err),
              });
          ...
          if (action === 'config' && req.method === 'GET') {
            const dressing = getInstalledDressingByProviderKey(providerKey);
            if (!dressing) {
              sendJson(res, 404, { error: 'Not installed' });
              return;
            }
            const config = dressing.config_json
              ? JSON.parse(dressing.config_json)
              : {};
            const schema = dressing.config_schema
              ? JSON.parse(dressing.config_schema)
              : {};
            sendJson(res, 200, { config, schema });
            return;
          }
          ...
          if (action === 'config' && req.method === 'POST') {
            ...
            if (agentPlugin.stop) {
              try {
                await agentPlugin.stop();
              } catch {}
            }
            invalidatePluginCache(providerKey);
      ...
      // ── Local plugin management API ──
      if (url.pathname.startsWith('/api/dressing/')) {
        // GET /api/dressing/installed — 설치된 로컬/내장 플러그인 목록
        if (
          url.pathname === '/api/dressing/installed' &&
          req.method === 'GET'
        ) {
          const plugins = listInstalledDressings().map((d) => ({
            type: d.dressing_type,
            skill_id: d.dressing_skill_id,
            name: d.name,
            version: d.version,
        ...
        // PUT /api/dressing/plugin-config — 플러그인 설정 업데이트 (same-origin)
        if (
          url.pathname === '/api/dressing/plugin-config' &&
          req.method === 'PUT'
        ) {
          const body = await readJsonBody(req);
          const dressingId =
            typeof body.dressing_id === 'string' ? body.dressing_id : '';
          const configJson =
            typeof body.config_json === 'string' ? body.config_json : '';
          if (!dressingId) {
            sendJson(res, 400, { error: 'dressing_id required' });
            return;
          }
          updateInstalledDressingConfig(dressingId, configJson);
          sendJson(res, 200, { ok: true });
          return;
        }

        // DELETE /api/dressing/plugin — 플러그인 삭제 (same-origin)
        if (
          url.pathname === '/api/dressing/plugin' &&
          req.method === 'DELETE'
        ) {
          const body = await readJsonBody(req);
          const dressingId =
          ...
          const existing = listInstalledDressings().find(
            (d) => d.id === dressingId,
          );
          deleteInstalledDressing(dressingId);
          const { invalidatePluginCache } =
            await import('./dressing/plugin-loader.js');
          if (existing?.provider_key)
            invalidatePluginCache(existing.provider_key);
          if (existing?.channel_type)
            invalidatePluginCache(existing.channel_type);
          sendJson(res, 200, { ok: true });
          return;
        }
  ...
  server.on('error', (err: NodeJS.ErrnoException) => {
    if (err.code === 'EADDRINUSE') {
      logger.error(
        { host, port },
        `Web UI port ${port} already in use — skipping listen. Another instance may be running.`,
      );
    } else {
      logger.error({ err, host, port }, 'Web UI server error');
    }
  });
