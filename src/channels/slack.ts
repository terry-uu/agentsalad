 ...
 * 첨부파일: message.files → MessageAttachment[]로 변환.
 *   url_private_download + botToken(Bearer 인증)으로 다운로드.
...
export function createSlackChannel(config: SlackChannelConfig): Channel {
  ...
  app.message(async ({ message }) => {
    ...
    const rawText = hasText ? (message as { text: string }).text : '';
    const botMentionPattern = botUserId
      ? new RegExp(`<@${botUserId}>`, 'g')
      : null;
    const isMention =
      !isDM && botMentionPattern ? botMentionPattern.test(rawText) : false;
